 ...
 *  These tests will require Python3, as well as the pycryptodome module to be
 * installed.
 */
 ...
 * Used to dereference variables and free memory used during the python truth
 *baseline process. Must be called or tests could cause memory leaks, and
 *erroneous data.
 **/
int EndPython() {
 ...
 * @param expected Output character array that will be allocated within this
 *function.  Memory must be freed upon completion of the test
 * @param expected_length The length of the expected character array this is set
 *within this function
 * @note User must free memory themselves.
 **/
void python_cmac(char *data, char *key, uint8_t **expected,
 ...
 * Used to generate truth data for Authorized Encryption.  Results are compared
 *against TC_ApplySecurity Functionality, as well as in reverse using the
 *TC_ProcessSecurity function.
 ...
 * @param header Hextring of the header (AAD) that will be used during
 *encryption
 * @param bitmask Hexstring of the bitmask that will be used on the header
 * @param expected Ouput character array that will be allocated within this
 *function.  Memory must be freed upon completion of test.
 * @param expected_length The length of the expected character array that is set
 *within this function
 * @note The char** expected that is passsed to this function must be freed by
 *the user upon completion of unit test or other call.
 **/
void python_auth_encryption(char *data, char *key, char *iv, char *header,
                            char *bitmask, uint8_t **expected,
                            long *expected_length) {
  setup_python();
  pValue = PyObject_CallMethod(pInstance, "encrypt", "sssss", data, key, iv,
                               header, bitmask);
 ...
 * Utilizes the python_auth_encryption(uint8_t* data, uint8_t* key, uint8_t* iv,
 *uint8_t* header, uint8_t* bitmask, uint8_t** expected, long* expected_length)
 *function to create baseline truth data.  This data is then compared against
 *the generated tag and cipher text that is generated by the
 *Crypto_TC_ApplySecurity(const uint8_t* p_in_frame, const uint16_t
 *in_frame_length, uint8_t** pp_in_frame, uint16_t* p_enc_frame_len) function,
 *as well as the FECF.
 **/
/*
UTEST(ET_VALIDATION, AUTH_ENCRYPTION_TEST)
{
    ...
    char* activate_sa4_h =
"2003002000ff000100011880d2c9000e197f0b001b0004000400003040d95ecbc2"; char*
enc_test_ping_h = "2003041600ff1880d2ca0008197f0b0031000039c5082d";
    ...
    hex_conversion(enc_test_ping_h, (char**) &enc_test_ping_b,
&enc_test_ping_len);

    SecurityAssociation_t* test_association_1 = NULL;
    SecurityAssociation_t* test_association_4 = NULL;
    test_association_1 = malloc(sizeof(SecurityAssociation_t) *
sizeof(uint8_t)); test_association_4 = malloc(sizeof(SecurityAssociation_t) *
sizeof(uint8_t));
    ...
    return_val = Crypto_TC_ProcessSecurity(activate_sa4_b, &activate_sa4_len,
tc_sdls_processed_frame); ASSERT_EQ(CRYPTO_LIB_SUCCESS, return_val);
    ...
    return_val = Crypto_TC_ApplySecurity(enc_test_ping_b, enc_test_ping_len,
&ptr_enc_frame, &enc_frame_len); ASSERT_EQ(CRYPTO_LIB_SUCCESS, return_val);
    ...
    python_auth_encryption("1880d2ca0008197f0b0031000039c5",
                           ...
                           "000000000000000000000001", "2003043400FF0004", "00",
&expected, &expected_length); for (int i = 0; i < expected_length; i++)
    {
 ...
 * Makes use of truth data created from the previous AUTH_ENCRYPTION_TEST, to
 *validate that Crypto_TC_ProcessSecurity( uint8_t* ingest, int*
 *len_ingest,TC_t* tc_sdls_processed_frame) properly decrypts data and returns
 *it to the intial truth data created by the python_auth_encryption(uint8_t*
 *data, uint8_t* key, uint8_t* iv, uint8_t* header, uint8_t* bitmask, uint8_t**
 *expected, long* expected_length) function.
 **/
UTEST(DT_VALIDATION, AUTH_DECRYPTION_TEST) {
  ...
  char *activate_sa4_h =
      "2003002000ff000100011880d2c9000e197f0b001b0004000400003040d95ecbc2";
  char *dec_test_ping_h =
      "2003043400FF00040000000000000000000000017E1D8EEA8D45CEBA17888E0CDCD747DC"
      "78E5F372F997F2A63AA5DFC168395DC987";
  ...
  hex_conversion(activate_sa4_h, (char **)&activate_sa4_b, &activate_sa4_len);
  hex_conversion(dec_test_ping_h, (char **)&dec_test_ping_b,
                 &dec_test_ping_len);
  hex_conversion(enc_test_ping_h, (char **)&enc_test_ping_b,
                 &enc_test_ping_len);
  ...
  return_val = Crypto_TC_ProcessSecurity(dec_test_ping_b, &dec_test_ping_len,
                                         tc_sdls_processed_frame);
...
UTEST(NIST_ENC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0) {
  ...
  GvcidManagedParameters_t TC_UT_Managed_Parameters = {
      ...
      AOS_FHEC_NA, AOS_IZ_NA, 0, TC_NO_SEGMENT_HDRS,
      1024,        TC_OCF_NA, 1};
  ...
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_ct_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_ct_len,
      buffer_nist_key_len = 0;
  ...
  hex_conversion(buffer_nist_key_h, (char **)&buffer_nist_key_b,
                 &buffer_nist_key_len);
  ...
  hex_conversion(buffer_nist_pt_h, (char **)&buffer_nist_pt_b,
                 &buffer_nist_pt_len);
  // Convert/Set input IV
  hex_conversion(buffer_nist_iv_h, (char **)&buffer_nist_iv_b,
                 &buffer_nist_iv_len);
  memcpy(test_association->iv, buffer_nist_iv_b, buffer_nist_iv_len);
  // Convert input ciphertext
  hex_conversion(buffer_nist_ct_h, (char **)&buffer_nist_ct_b,
                 &buffer_nist_ct_len);

  Crypto_TC_ApplySecurity(buffer_nist_pt_b, buffer_nist_pt_len, &ptr_enc_frame,
                          &enc_frame_len);
  // Note: For comparison, interested in the TF payload (exclude headers and
  // FECF if present) Calc payload index: total length - pt length
  uint16_t enc_data_idx = enc_frame_len - buffer_nist_ct_len - 2;
  Crypto_Shutdown();
  for (int i = 0; i < buffer_nist_pt_len - 7; i++) {
    // printf("[%d]: %02x -> %02x \n", i, *(ptr_enc_frame + enc_data_idx),
    // buffer_nist_ct_b[i]);
...
UTEST(NIST_DEC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0) {
  ...
  char *buffer_nist_et_h = "2003002500FF0009B6AC8E4963F49207FFD6374C00001224DFE"
                           "FB72A20D49E09256908874979AD6F";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_et_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_et_len,
      buffer_nist_key_len = 0;
  ...
  hex_conversion(buffer_nist_et_h, (char **)&buffer_nist_et_b,
                 &buffer_nist_et_len);

  Crypto_TC_ProcessSecurity(buffer_nist_et_b, &buffer_nist_et_len,
  ...
  for (int i = 0; i < tc_nist_processed_frame->tc_pdu_len; i++) {
    if (buffer_nist_pt_b[i + 5] != tc_nist_processed_frame->tc_pdu[i]) {
      printf("[%d]: %02x -> %02x \n", i, buffer_nist_pt_b[i + 5],
             tc_nist_processed_frame->tc_pdu[i]);
    }
...
UTEST(NIST_DEC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_1) {
  ...
  char *buffer_nist_et_h = "2003002500FF00091AF2613C4184DBD101FCEDCE00009CD21F4"
                           "14F1F54D5F6F58B1F2F77E5B66987";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_et_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_et_len,
      buffer_nist_key_len = 0;
  ...
  status = Crypto_TC_ProcessSecurity(buffer_nist_et_b, &buffer_nist_et_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(0, status);
...
UTEST(NIST_DEC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_2) {
  ...
  char *buffer_nist_et_h = "2003002500FF00096805BE41E983717BF67810520000487211D"
                           "D440F4D09D00BC5C3158A822C46E3";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_et_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_et_len,
      buffer_nist_key_len = 0;
...
UTEST(NIST_DEC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_3) {
  ...
  char *buffer_nist_et_h = "2003002500FF0009F0B744F157087DF4E41818A90000B65A287"
                           "8B9DDDBD4A0204DAE6A6A6FC0C327";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_et_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_et_len,
      buffer_nist_key_len = 0;
...
UTEST(NIST_DEC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_4) {
  ...
  char *buffer_nist_et_h = "2003002500FF0009EEDDEAF4355C826DFD15339300005C6CFBD"
                           "D06C19445ECF500C21AECA1738A7D";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_et_b,
      *buffer_nist_key_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_et_len,
      buffer_nist_key_len = 0;
 ...
 * 1) Ensure a valid implementation - Utilize Cyberchef's AES Encrypt to
 *re-create a NIST test vector with AAD 2) Generate Truth data - Use same
 *CyberChef settings on a created TF 3) Validate Cryptolib output with CyberChef
 *output Bit Mask of zeros Bit-mask of zeros in this test for a total length of:
...
UTEST(NIST_ENC_MAC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0) {
  ...
  char *buffer_nist_aad_h =
      "b96baa8c1c75a671bfb2d08d06be5f36"; // Zeroed out by abm
  char *buffer_cyber_chef_mac_h = "b04cb89841f1591793f6e1bca1479d92";
  uint8_t *buffer_nist_pt_b, *buffer_nist_iv_b, *buffer_nist_key_b,
      *buffer_nist_aad_b, *buffer_cyber_chef_mac_b = NULL;
  int buffer_nist_pt_len, buffer_nist_iv_len, buffer_nist_key_len,
      buffer_nist_aad_len, buffer_cyber_chef_mac_len = 0;
  ...
  test_association->iv_len = 12;
  test_association->abm_len = 1024;
  memset(test_association->abm, 0xFF,
         (test_association->abm_len * sizeof(uint8_t))); // Bitmask
  test_association->stmacf_len = 16;
  test_association->arsnw_len = 1;
  test_association->arsnw = 5;
  test_association->arsn_len = 2;
  ...
  memcpy(test_association->iv, buffer_nist_iv_b, buffer_nist_iv_len);
  // Convert input aad
  hex_conversion(buffer_nist_aad_h, (char **)&buffer_nist_aad_b,
                 &buffer_nist_aad_len);
  // Convert input mac
  hex_conversion(buffer_cyber_chef_mac_h, (char **)&buffer_cyber_chef_mac_b,
                 &buffer_cyber_chef_mac_len);
...
UTEST(NIST_DEC_MAC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0) {
  ...
  //  |  Header | SPI |           iv               |         plaintext | mac
  //  |fecf|
  char *buffer_nist_mac_frame_h =
      "2003003700FF0009D79CF22D504CC793C3FB6C8A0000722ee47da4b77424733546c2d400"
      "c4e5E7BB31D506998FCBDA078E66D5680194B401";

  uint8_t *buffer_nist_iv_b, *buffer_nist_pt_b, *buffer_nist_key_b,
      *buffer_cyber_chef_mac_b, *buffer_nist_mac_frame_b,
      *buffer_nist_cp_b = NULL;
  int buffer_nist_iv_len, buffer_nist_pt_len, buffer_nist_key_len,
      buffer_cyber_chef_mac_len, buffer_nist_mac_frame_len = 0;
  ...
  test_association->abm_len = 1024;
  memset(test_association->abm, 0xFF,
         (test_association->abm_len * sizeof(uint8_t)));
  test_association->shivf_len = 12;
  test_association->iv_len = 12;
  test_association->stmacf_len = 16;
  test_association->sa_state = SA_OPERATIONAL;
  ...
  hex_conversion(buffer_cyber_chef_mac_h, (char **)&buffer_cyber_chef_mac_b,
                 &buffer_cyber_chef_mac_len);
  // Convert mac frame
  hex_conversion(buffer_nist_mac_frame_h, (char **)&buffer_nist_mac_frame_b,
                 &buffer_nist_mac_frame_len);

  Crypto_TC_ProcessSecurity(buffer_nist_mac_frame_b, &buffer_nist_mac_frame_len,
                            tc_nist_processed_frame);
  ...
  // Verify the MAC
  for (int i = 0; i < test_association->stmacf_len; i++) {
    ASSERT_EQ(tc_nist_processed_frame->tc_sec_trailer.mac[i],
              buffer_cyber_chef_mac_b[i]);
  }
  for (int i = 0; i < tc_nist_processed_frame->tc_pdu_len;
       i++) { // Verify the PDU Data is present and not stomped
...
UTEST(NIST_DEC_MAC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0_BAD_DATA) {
  ...
  // Create a MAC'd frame by adding our headers and a fecf
  //  |  Header | SPI |           iv          |         plaintext             |
  //  mac               |fecf|
  char *buffer_nist_mac_frame_h =
      "2003003700FF0009D79CF22D504CC793C3FB6C8A0000722ee47da4b77424733546c2d400"
      "c40034d0e323f5e4b80426401d4aa37930da123b";
  ...
  status = Crypto_TC_ProcessSecurity(buffer_nist_mac_frame_b,
                                     &buffer_nist_mac_frame_len,
                                     tc_nist_processed_frame);
  // printf("TC_Process returned status %d\n", status);

  // Note: For comparison, interested in the TF payload (exclude headers and
  // FECF if present) Calc payload index: total length - pt length #ifdef DEBUG
  //     printf("Expected MAC: ");
  //     for (int i=0; i<buffer_cyber_chef_mac_len; i++)
  //     {
  //         printf("%02x ", buffer_cyber_chef_mac_b[i]);
...
UTEST(NIST_DEC_MAC_VALIDATION, AES_GCM_256_IV_96_PT_128_TEST_0_BAD_MAC) {
  ...
  //  mac               |fecf|
  char *buffer_nist_mac_frame_h =
      "2003003500FF0009D79CF22D504CC793C3FB6C8A722ee47da4b77424733546c2d400c4e5"
      "34d0e323f5e4b80426401d4aa37930009f68";
...
UTEST(NIST_ENC_CMAC_VALIDATION, AES_CMAC_256_PT_128_TEST_0) {
  ...
  char *buffer_frame_pt_h =
      "2003004600C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A696DB411E47C0"
      "E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19394258C925";
  // Python truth string passed below is ZEROed out, not including a MAC or FECF
  // which isn't hashed against, but the LENGTH (including fecf) needs to be
  // updated in the Tf Header Length is dependent on whatever the variable mac
  ...
  test_association->abm_len = 1024;
  memset(test_association->abm, 0x00,
         (test_association->abm_len * sizeof(uint8_t))); // Bitmask
  test_association->stmacf_len = 16;
  test_association->sa_state = SA_OPERATIONAL;
  test_association->ecs = CRYPTO_CIPHER_NONE;
  ...
  // Convert input plaintext
  hex_conversion(buffer_frame_pt_h, (char **)&buffer_frame_pt_b,
                 &buffer_frame_pt_len);
  // Convert input mac
  hex_conversion(buffer_python_mac_h, (char **)&buffer_python_mac_b,
                 &buffer_python_mac_len);

  Crypto_TC_ApplySecurity(buffer_frame_pt_b, buffer_frame_pt_len,
  ...
  for (int i = 0; i < buffer_python_mac_len; i++) {
    if (ptr_enc_frame[enc_data_idx] != buffer_python_mac_b[i]) {
      printf("[%d] Truth: %02x, Actual: %02x\n", enc_data_idx,
             buffer_python_mac_b[i], ptr_enc_frame[enc_data_idx]);
    }
...
UTEST(NIST_ENC_CMAC_VALIDATION, AES_CMAC_256_PT_128_TEST_1) {
  ...
  char *buffer_frame_pt_h =
      "2003004600C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A696DB411E47C0"
      "E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19394258C925";
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatevr the variable mac length to be
  ...
  int status = Crypto_TC_ApplySecurity(buffer_frame_pt_b, buffer_frame_pt_len,
                                       &ptr_enc_frame, &enc_frame_len);
...
UTEST(NIST_DEC_CMAC_VALIDATION, AES_CMAC_256_PT_128_TEST_0) {
  ...
  char *buffer_nist_key_h =
      "b228c753292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445";
  //                        | Header  |SPI|  ARSN  |           NIST CMAC Test
  //                        Vector |   MAC                        |FECF|
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "3942587629961f6b92145290ad3e149940511a46ce";
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatevr the variable mac length to be
  // updated in the header
  ...
  int32_t status = Crypto_TC_ProcessSecurity(
      buffer_frame_pt_b, &buffer_frame_pt_len, tc_sdls_processed_frame);
  ...
  for (int i = 0; i < buffer_python_mac_len; i++) {
    printf("[%d] Truth: %02x, Actual: %02x\n", enc_data_idx,
           buffer_python_mac_b[i],
           (tc_sdls_processed_frame->tc_sec_trailer.mac[i]));
    ASSERT_EQ(tc_sdls_processed_frame->tc_sec_trailer.mac[i],
              buffer_python_mac_b[i]);
    enc_data_idx++;
  }
...
UTEST(NIST_DEC_CMAC_VALIDATION, AES_CMAC_256_PT_128_TEST_1) {
  ...
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "394258CF549CC15D63EAB7AD25EB3089D94E6C2D9D";
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatevr the variable mac length to be
  // updated in the header
  ...
  Crypto_TC_ProcessSecurity(buffer_frame_pt_b, &buffer_frame_pt_len,
                            tc_sdls_processed_frame);
...
UTEST(NIST_ENC_HMAC_VALIDATION, SHA_256_PT_128_TEST_1) {
  ...
  status = Crypto_TC_ApplySecurity(buffer_frame_pt_b, buffer_frame_pt_len,
                                   &ptr_enc_frame, &enc_frame_len);
  ASSERT_EQ(status, CRYPTO_LIB_SUCCESS);
...
UTEST(NIST_ENC_HMAC_VALIDATION, SHA_512_PT_128_TEST_0) {
  ...
  // NOTE: Added Transfer Frame header to the plaintext
  char *buffer_nist_key_h =
      "b228c753292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445b228c753"
      "292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445";
  //                        |  Header |           NIST CMAC Test Vector |FECF|
  char *buffer_frame_pt_h =
      "2003004600C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A696DB411E47C0"
      "E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19394258C925";
...
UTEST(NIST_ENC_HMAC_VALIDATION, SHA_512_PT_128_TEST_1) {
  ...
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatever the variable mac length to be
  // updated in the header
  //  | Header |SPI|  ARSN  | NIST CMAC Frame Data |
  // "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A696DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19394258";
  // Python output MAC
  ...
  for (int i = 0; i < buffer_python_mac_len; i++) {
    printf("[%d] Truth: %02x, Actual: %02x\n", enc_data_idx,
           buffer_python_mac_b[i], *(ptr_enc_frame + enc_data_idx + i));
...
UTEST(NIST_DEC_HMAC_VALIDATION, SHA_256_PT_128_TEST_0) {
  ...
  char *buffer_nist_key_h =
      "b228c753292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445";
  //                        |  Header |SPI | arsn   |     Payload | SHA 256 HMAC
  //                        |FECF|
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "3942586B5953E41CFFB4D15A62E25DA5E092F969F2";
  // Python truth string passed below is ZEROed out, not including a MAC or FECF
  // which isn't hashed against, but the LENGTH (including fecf) needs to be
  // updated in the Tf Header Length is dependent on whatever the variable mac
  // length to be updated in the header
  ...
  status = Crypto_TC_ProcessSecurity(buffer_frame_pt_b, &buffer_frame_pt_len,
                                     tc_sdls_processed_frame);
  ASSERT_EQ(status, CRYPTO_LIB_SUCCESS);
  ...
  for (int i = 0; i < buffer_python_mac_len; i++) {
    if (*(tc_sdls_processed_frame->tc_sec_trailer.mac + i) !=
        buffer_python_mac_b[i]) {
...
UTEST(NIST_DEC_HMAC_VALIDATION, SHA_256_PT_128_TEST_1) {
  ...
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "3942585d0ae5c3859d51d9c1e31681db475acb5b35";
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatever the variable mac length to be
  // updated in the header.
...
UTEST(NIST_DEC_HMAC_VALIDATION, SHA_512_PT_128_TEST_0) {
  ...
  char *buffer_nist_key_h =
      "b228c753292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445b228c753"
      "292acd5df351000a591bf960d8555c3f6284afe7c6846cbb6c6f5445";
  //                        |  Header |SPI | ARSN   |     Payload | SHA 512 HMAC
  //                        |FECF|
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "394258676e9ebdf306b7db7ad41892887342e80DC5";
  // Python truth string passed below is ZEROed out, not including a MAC or FECF
  // which isn't hashed against, but the LENGTH (including fecf) needs to be
  // updated in the Tf Header Length is dependent on whatever the variable mac
  // length to be updated in the header
...
UTEST(NIST_DEC_HMAC_VALIDATION, SHA_512_PT_128_TEST_1) {
  ...
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "394258907bbd1d9f2fd37e541f0b1ee12f5db0679a";
  // Python truth string passed below is ZEROed out, not including a MAC or FECF
  // which isn't hashed against, but the LENGTH (including fecf) needs to be
  // updated in the Tf Header Length is dependent on whatever the variable mac
  // length to be updated in the header
...
/**
 * @brief Unit Test: Encrypts a frame, then decrypts the output to ensure the
 *reverse doesn't error
 **/
UTEST(PLAINTEXT, ENCRYPT_DECRYPT) {
  ...
  Crypto_Config_CryptoLib(
      KEY_TYPE_INTERNAL, MC_TYPE_INTERNAL, SA_TYPE_INMEMORY,
      CRYPTOGRAPHY_TYPE_LIBGCRYPT, IV_INTERNAL, CRYPTO_TC_CREATE_FECF_TRUE,
      TC_PROCESS_SDLS_PDUS_TRUE, TC_HAS_PUS_HDR, TC_IGNORE_SA_STATE_TRUE,
      TC_IGNORE_ANTI_REPLAY_TRUE, TC_UNIQUE_SA_PER_MAP_ID_FALSE,
      TC_CHECK_FECF_TRUE, 0x3F, SA_INCREMENT_NONTRANSMITTED_IV_TRUE);
  // Crypto_Config_Add_Gvcid_Managed_Parameter(0, 0x0003, 0, TC_HAS_FECF,
  // TC_NO_SEGMENT_HDRS, TC_OCF_NA, 1024, AOS_FHEC_NA, AOS_IZ_NA, 0);
  ...
  char *jpl_frame_pt_h =
      "2003001c00ff000100001880d03e000a197f0b000300020093d4ba21c4555555555555";
  ...
  status = Crypto_TC_ApplySecurity(jpl_frame_pt_b, jpl_frame_pt_len,
                                   &ptr_enc_frame, &enc_frame_len);
  ASSERT_EQ(status, CRYPTO_LIB_SUCCESS);
  ...
  status =
      Crypto_TC_ProcessSecurity(ptr_enc_frame, &len, tc_sdls_processed_frame);
  Crypto_Shutdown();
  ASSERT_EQ(status, CRYPTO_LIB_SUCCESS);
}
...
UTEST(NIST_DEC_HMAC_VALIDATION, SHA_512_SHORT_KEY) {
  ...
  char *buffer_frame_pt_h =
      "2003005C00000900000000C66D322247EBF272E6A353F9940B00847CF78E27F2BC0C81A6"
      "96DB411E47C0E9630137D3FA860A71158E23D80B699E8006E52345FB7273B2E084407F19"
      "39425875c570016a9458a71cea6aaca6ff46970f67";
  // Python truth string passed below, not including a MAC or FECF which isn't
  // hashed against, but the LENGTH (including fecf) needs to be updated in the
  // Tf Header Length is dependent on whatever the variable mac length to be
  // updated in the header
