...
UTEST(TC_PROCESS, EXERCISE_IV) {
  ...
  char *buffer_nist_iv_h =
      "b6ac8e4963f49207ffd6374b"; // The last valid IV that was seen by the SA
  char *buffer_replay_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374B1224DFEFB72A20D49E09256908874979"
      "DFC1"; // IV is one less than library expects
  char *buffer_outside_window_h =
      "2003002500FF0009B6AC8E4963F49207FFD6375C1224DFEFB72A20D49E09256908874979"
      "B36E"; // IV is outside the positive window
  char *buffer_good_iv_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374C1224DFEFB72A20D49E09256908874979"
      "AD6F"; // IV is the next one expected
  char *buffer_good_iv_with_gap_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374F1224DFEFB72A20D49E09256908874979"
      "9C49"; // IV is valid, but not next one expected
  uint8_t *buffer_replay_b, *buffer_outside_window_b, *buffer_good_iv_b,
      *buffer_good_iv_with_gap_b, *buffer_nist_iv_b, *buffer_nist_key_b = NULL;
  int buffer_replay_len, buffer_outside_window_len, buffer_good_iv_len,
      buffer_good_iv_with_gap_len, buffer_nist_iv_len, buffer_nist_key_len = 0;
  ...
  hex_conversion(buffer_replay_h, (char **)&buffer_replay_b,
                 &buffer_replay_len);
  hex_conversion(buffer_outside_window_h, (char **)&buffer_outside_window_b,
                 &buffer_outside_window_len);
  hex_conversion(buffer_good_iv_h, (char **)&buffer_good_iv_b,
                 &buffer_good_iv_len);
  hex_conversion(buffer_good_iv_with_gap_h, (char **)&buffer_good_iv_with_gap_b,
                 &buffer_good_iv_with_gap_len);
  ...
  status = Crypto_TC_ProcessSecurity(buffer_replay_b, &buffer_replay_len,
  ...
  status = Crypto_TC_ProcessSecurity(buffer_outside_window_b,
                                     &buffer_outside_window_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_IV_OUTSIDE_WINDOW, status);
  ...
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_b, &buffer_good_iv_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);

  // Expect success on valid IV within window, but has a gap
  printf(KGRN "Checking valid IV within window... should be able to receive "
              "it... \n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_with_gap_b,
                                     &buffer_good_iv_with_gap_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
  ...
  for (int i = 0; i < test_association->shivf_len; i++) {
    ASSERT_EQ(*(test_association->iv + i), *(buffer_good_iv_with_gap_b + 8 +
                                             i)); // 8 is IV offset into packet
...
UTEST(TC_PROCESS, EXERCISE_ARSN) {
  ...
  // For reference:        | Header  |SH SPI SN| Payload                       |
  // MAC                           |FECF
  char *buffer_replay_h =
      "2003002B00FF000901231224DFEFB72A20D49E09256908874979fd56ca1ffc2697a700db"
      "e6292c10e9ef1B49"; // ARSN is one less than library expects
  char *buffer_outside_window_h =
      "2003002B00FF000904441224DFEFB72A20D49E09256908874979fd56ca1ffc2697a700db"
      "e6292c10e9ef9C5C"; // ARSN is outside the positive window
  char *buffer_good_arsn_h =
      "2003002B00FF000901241224DFEFB72A20D49E09256908874979fd56ca1ffc2697a700db"
      "e6292c10e9ef8A3E"; // ARSN is the next one expected
  char *buffer_good_arsn_with_gap_h =
      "2003002B00FF000901291224DFEFB72A20D49E09256908874979fd56ca1ffc2697a700db"
      "e6292c10e9ef3EB4"; // ARSN is valid, but not next one expected
  uint8_t *buffer_replay_b, *buffer_outside_window_b, *buffer_good_arsn_b,
      *buffer_good_arsn_with_gap_b, *buffer_arsn_b, *buffer_nist_key_b = NULL;
  int buffer_replay_len, buffer_outside_window_len, buffer_good_arsn_len,
      buffer_good_arsn_with_gap_len, buffer_arsn_len, buffer_nist_key_len = 0;
  ...
  // memset(test_association->abm, 0x00, (test_association->abm_len *
  // sizeof(uint8_t)));
  ...
  hex_conversion(buffer_outside_window_h, (char **)&buffer_outside_window_b,
                 &buffer_outside_window_len);
  hex_conversion(buffer_good_arsn_h, (char **)&buffer_good_arsn_b,
                 &buffer_good_arsn_len);
  hex_conversion(buffer_good_arsn_with_gap_h,
                 (char **)&buffer_good_arsn_with_gap_b,
                 &buffer_good_arsn_with_gap_len);
  ...
  printf(
      KGRN
      "Checking next valid ARSN... should be able to receive it... \n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_arsn_b, &buffer_good_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);

  // Expect success on valid ARSN within window, but has a gap
  printf(KGRN "Checking valid ARSN within window... should be able to receive "
              "it... \n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_arsn_with_gap_b,
                                     &buffer_good_arsn_with_gap_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
...
UTEST(TC_PROCESS, HAPPY_PATH_PROCESS_STATIC_IV_ROLLOVER) {
  ...
  char *dec_test_fe_h = "2003002D00000004FFFFFFFFFFFE610B082EA91C8AA93F08EAA642"
                        "EA3189128D87159B2354AA753248F050022FD9";
  char *dec_test_ff_h = "2003002D00000004FFFFFFFFFFFFCECBA30A6E0B54ACE0D5F92D13"
                        "60084822CFA46240C0CD7D6830A6A7771ECFEC";
  char *dec_test_00_h = "2003002D0000000400000000000064DB31BBC4656F072A8E4A706F"
                        "9508C440A003496E8A71FD47621297DDCC393C";
  ...
  return_val = Crypto_TC_ProcessSecurity(dec_test_fe_b, &dec_test_fe_len,
                                         &tc_sdls_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, return_val);
  ASSERT_EQ(test_association->iv[11], 0xFE);

  return_val = Crypto_TC_ProcessSecurity(dec_test_ff_b, &dec_test_ff_len,
                                         &tc_sdls_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, return_val);
  ASSERT_EQ(test_association->iv[11], 0xFF);

  return_val = Crypto_TC_ProcessSecurity(dec_test_00_b, &dec_test_00_len,
                                         &tc_sdls_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, return_val);
...
UTEST(TC_PROCESS, HAPPY_PATH_PROCESS_NONTRANSMITTED_INCREMENTING_IV_ROLLOVER) {
  ...
  char *dec_test_ff_h = "2003002D00000004FFFFFFFFFFFFCECBA30A6E0B54ACE0D5F92D13"
                        "60084822CFA46240C0CD7D6830A6A7771ECFEC";
  char *dec_test_00_h = "2003002D00000004000000000000CEB2378F0F335664496406AC4F"
                        "3A2ABFFD8678CB76DD009D7FE5B425BB96F567";
...
UTEST(TC_PROCESS,
  ...
  char *dec_test_fe_h = "2003002900000004FFFE80D2C70008197F0B00310000B1FE7F9781"
                        "6F523951BAF0445DB078B502760741";
  char *dec_test_ff_h = "2003002900000004FFFF80D2C70008197F0B00310000B1FE7F9781"
                        "6F523951BAF0445DB078B502768968";
  char *dec_test_00_h = "2003002900000004000080D2C70008197F0B00310000B1FE7F9781"
                        "6F523951BAF0445DB078B50276E797";
  ...
  // This TA was originally setup for AESGCM, need to specify an akid so we can
  // use it for a MAC
...
UTEST(TC_PROCESS, ERROR_TC_INPUT_FRAME_TOO_SHORT_FOR_SPEC) {
  ...
  Crypto_Config_CryptoLib(
      ...
      TC_CHECK_FECF_TRUE, 0x3F, SA_INCREMENT_NONTRANSMITTED_IV_TRUE);
  // Crypto_Config_Add_Gvcid_Managed_Parameter(0, 0x0003, 0, TC_HAS_FECF,
  // TC_HAS_SEGMENT_HDRS, TC_OCF_NA, 4, AOS_FHEC_NA, AOS_IZ_NA, 0);
  GvcidManagedParameters_t TC_UT_Managed_Parameters = {
      0,           0x0003,    0, TC_HAS_FECF,
      AOS_FHEC_NA, AOS_IZ_NA, 0, TC_HAS_SEGMENT_HDRS,
      1024,        TC_OCF_NA, 1};
...
UTEST(TC_PROCESS,
      ERROR_TC_INPUT_FRAME_TOO_SHORT_FOR_SPECIFIED_FRAME_LENGTH_HEADER) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  // Setup & Initialize CryptoLib
  Crypto_Config_CryptoLib(
      KEY_TYPE_INTERNAL, MC_TYPE_INTERNAL, SA_TYPE_INMEMORY,
  ...
  char *test_frame_pt_h = "200304260000020000000000000000000000309e09deeaa37548"
                          "7983a89f3ed7519a230baf22";
  uint8_t *test_frame_pt_b = NULL;
  int test_frame_pt_len = 0;
  ...
  status = Crypto_TC_ProcessSecurity(test_frame_pt_b, &test_frame_pt_len,
                                     tc_sdls_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_INPUT_FRAME_LENGTH_SHORTER_THAN_FRAME_HEADERS_LENGTH,
            status);
...
UTEST(TC_PROCESS, HAPPY_PATH_DECRYPT_CBC) {
  ...
  char *test_frame_pt_h = "2003002A0000000B000000000000000000000000000000000253"
                          "64F9BC3344AF359DA06CA886746F59A0AB";
  uint8_t *test_frame_pt_b = NULL;
  int test_frame_pt_len = 0;
...
UTEST(TC_PROCESS, DECRYPT_CBC_1B) {
  ...
  char *test_frame_pt_h = "2003002A0000000B00000000000000000000000000000000011C"
                          "1741A95DE7EF6FCF2B20B6F09E9FD29988";
  uint8_t *test_frame_pt_b = NULL;
  int test_frame_pt_len = 0;
...
UTEST(TC_PROCESS, DECRYPT_CBC_16B) {
  ...
  char *test_frame_pt_h =
      "2003003A0000000B00000000000000000000000000000000103970EAE4C05ACD1B0C348F"
      "DA174DF73EF0E2D603996C4B78B992CD60918729D3A47A";
  uint8_t *test_frame_pt_b = NULL;
  int test_frame_pt_len = 0;
...
UTEST(TC_PROCESS, GCM_IV_AND_ARSN) {
  ...
  Crypto_Config_CryptoLib(
      ...
      TC_IGNORE_ANTI_REPLAY_FALSE, TC_UNIQUE_SA_PER_MAP_ID_FALSE,
      TC_CHECK_FECF_FALSE, 0x3F, SA_INCREMENT_NONTRANSMITTED_IV_TRUE);
  // Crypto_Config_Add_Gvcid_Managed_Parameter(0, 0x0003, 0, TC_NO_FECF,
  // TC_HAS_SEGMENT_HDRS, TC_OCF_NA, 1024, AOS_FHEC_NA, AOS_IZ_NA, 0);
  // Crypto_Config_Add_Gvcid_Managed_Parameter(0, 0x0003, 1, TC_NO_FECF,
  // TC_HAS_SEGMENT_HDRS, TC_OCF_NA, 1024, AOS_FHEC_NA, AOS_IZ_NA, 0);

  GvcidManagedParameters_t TC_UT_Managed_Parameters = {
      0,           0x0003,    0, TC_NO_FECF,
      AOS_FHEC_NA, AOS_IZ_NA, 0, TC_HAS_SEGMENT_HDRS,
      1024,        TC_OCF_NA, 1};
  Crypto_Config_Add_Gvcid_Managed_Parameters(TC_UT_Managed_Parameters);
  TC_UT_Managed_Parameters.vcid = 1;
  ...
  char *buffer_bad_iv_bad_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374C01231224DFEFB72A20D49E0925690887"
      "4979"; // IV isa replay, ARSN is a replay
  char *buffer_good_iv_bad_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374C01231224DFEFB72A20D49E0925690887"
      "4979"; // IV is the next one expected, ARSN is a replay
  char *buffer_bad_iv_good_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374B01241224DFEFB72A20D49E0925690887"
      "4979"; // IV is a replay, ARSN is next expected
  char *buffer_good_iv_with_gap_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374F1224DFEFB72A20D49E09256908874979"
      "9C49"; // IV is valid, but not next one expected
  char *buffer_high_iv_good_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374B01241224DFEFB72A20D49E0925690887"
      "4979"; // IV is outside upper bounds, ARSN is next expected
  char *buffer_good_iv_high_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374BFFFE1224DFEFB72A20D49E0925690887"
      "4979"; // IV is next expected, ARSN is outside upper bounds
  char *buffer_good_iv_good_arsn_h =
      "2003002500FF0009B6AC8E4963F49207FFD6374C01241224DFEFB72A20D49E0925690887"
      "4979"; // IV is next expected, ARSN is next expected
  char *buffer_good_iv_gap_good_arsn_gap_h =
      "2003002500FF0009B6AC8E4963F49207FFD6375101291224DFEFB72A20D49E0925690887"
      "4979"; // IV is next expected, ARSN is next expected

  uint8_t *buffer_replay_b, *buffer_outside_window_b, *buffer_bad_iv_bad_arsn_b,
      *buffer_good_iv_bad_arsn_b, *buffer_bad_iv_good_arsn_b,
      *buffer_good_iv_with_gap_b, *buffer_high_iv_good_arsn_b,
      *buffer_good_iv_high_arsn_b, *buffer_nist_iv_b, *buffer_nist_key_b,
      *buffer_good_iv_good_arsn_b, *buffer_good_iv_gap_good_arsn_gap_b,
      *buffer_arsn_b = NULL;
  int buffer_replay_len, buffer_outside_window_len, buffer_bad_iv_bad_arsn_len,
      buffer_good_iv_bad_arsn_len, buffer_bad_iv_good_arsn_len,
      buffer_good_iv_with_gap_len, buffer_high_iv_good_arsn_len,
      buffer_good_iv_high_arsn_len, buffer_nist_iv_len, buffer_nist_key_len,
      buffer_good_iv_good_arsn_len, buffer_good_iv_gap_good_arsn_gap_len,
      buffer_arsn_len = 0;
  ...
  hex_conversion(buffer_outside_window_h, (char **)&buffer_outside_window_b,
                 &buffer_outside_window_len);
  hex_conversion(buffer_bad_iv_bad_arsn_h, (char **)&buffer_bad_iv_bad_arsn_b,
                 &buffer_bad_iv_bad_arsn_len);
  hex_conversion(buffer_good_iv_bad_arsn_h, (char **)&buffer_good_iv_bad_arsn_b,
                 &buffer_good_iv_bad_arsn_len);
  hex_conversion(buffer_bad_iv_good_arsn_h, (char **)&buffer_bad_iv_good_arsn_b,
                 &buffer_bad_iv_good_arsn_len);
  hex_conversion(buffer_good_iv_with_gap_h, (char **)&buffer_good_iv_with_gap_b,
                 &buffer_good_iv_with_gap_len);
  hex_conversion(buffer_high_iv_good_arsn_h,
                 (char **)&buffer_high_iv_good_arsn_b,
                 &buffer_high_iv_good_arsn_len);
  hex_conversion(buffer_good_iv_high_arsn_h,
                 (char **)&buffer_good_iv_high_arsn_b,
                 &buffer_good_iv_high_arsn_len);
  hex_conversion(buffer_good_iv_good_arsn_h,
                 (char **)&buffer_good_iv_good_arsn_b,
                 &buffer_good_iv_good_arsn_len);
  hex_conversion(buffer_good_iv_gap_good_arsn_gap_h,
                 (char **)&buffer_good_iv_gap_good_arsn_gap_b,
  ...
  printf(KGRN "Checking replay - using previous received ARSN and previous "
              "IV...\n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_bad_iv_bad_arsn_b,
                                     &buffer_bad_iv_bad_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_ARSN_OUTSIDE_WINDOW, status);
  ...
  printf(KGRN "Checking replay - using previous received ARSN...\n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_bad_arsn_b,
                                     &buffer_good_iv_bad_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_ARSN_OUTSIDE_WINDOW, status);
  ...
  printf(KGRN "Checking replay - using previous received IV...\n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_bad_iv_good_arsn_b,
                                     &buffer_bad_iv_good_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_IV_OUTSIDE_WINDOW, status);
  ...
  // Check w/ Mike
  printf(KGRN
         "Checking replay - using IV outside (above) the window...\n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_high_iv_good_arsn_b,
                                     &buffer_high_iv_good_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_IV_OUTSIDE_WINDOW, status);
  ...
  // Check w/ Mike
  printf(KGRN
         "Checking replay - using ARSN outside (above) the window...\n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_high_arsn_b,
                                     &buffer_good_iv_high_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_ERR_ARSN_OUTSIDE_WINDOW, status);

  // Expect success on next valid IV && ARSN
  printf(KGRN "Checking  next valid IV && valid ARSN... should be able to "
              "receive it... \n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_good_arsn_b,
                                     &buffer_good_iv_good_arsn_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
  ...
  printf(KGRN "Checking valid IV and ARSN within window... should be able to "
              "receive it... \n" RESET);
  status = Crypto_TC_ProcessSecurity(buffer_good_iv_gap_good_arsn_gap_b,
                                     &buffer_good_iv_gap_good_arsn_gap_len,
                                     tc_nist_processed_frame);
  ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
  ...
  for (int i = 0; i < test_association->shivf_len; i++) {
    ASSERT_EQ(*(test_association->iv + i),
              *(buffer_good_iv_gap_good_arsn_gap_b + 8 +
                i)); // 8 is IV offset into packet
    printf("%02X", *(test_association->iv + i));
  }
  printf("\n");
...
UTEST(TC_PROCESS, TC_SA_SEGFAULT_TEST) {
  ...
  // Test frame setup
  char *test_frame_pt_h = "2003002A000000FF000000000000000000000000000000000253"
                          "64F9BC3344AF359DA06CA886748F59A0AB";
  uint8_t *test_frame_pt_b = NULL;
  int test_frame_pt_len = 0;
