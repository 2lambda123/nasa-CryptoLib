/* Copyright (C) 2009 - 2022 National Aeronautics and Space Administration. All
  Foreign Rights are Reserved to the U.S. Government. This software is provided
  "as is" without any warranty of any kind, either expressed, implied, or
  statutory, including, but not limited to, any warranty that the software will
  conform to specifications, any implied warranties of merchantability, fitness
  for a particular purpose, and freedom from infringement, and any warranty that
  the documentation will conform to the program, or any warranty that the
  software will be error free. In no event shall NASA be liable for any damages,
  including, but not limited to direct, indirect, special or consequential
  damages, arising out of, resulting from, or in any way connected with the
  software or its documentation, whether or not based upon warranty, contract,
  tort or otherwise, and whether or not loss was sustained from, or arose out of
  the results of, or use of, the software, documentation or services provided
  hereunder. ITC Team NASA IV&V jstar-development-team@mail.nasa.gov
 */
 ...
 *1) Required development packages: sudo yum install
mariadb-connector-c-devel.x86_64
 *
  2)IMPORTANT:The .pem files & MySQL MUST be configured and on the host before
running this test
 * using this test refer to the
guide:https://dev.mysql.com/doc/refman/8.0/en/encrypted-connections.html
 *
 ...
 *
 IMPORTANT:The database must have similar configuration for this test to
succeed: MariaDB server to use the standard host-based <your_server_hots_name>.
In our case host was asec-cmdenc-dev2.jpl.nasa.gov
This was done by editing the server configuration file --
/etc/my.cnf.d/mariadb.server -- and adding the following options:
...
This user is allowed to access MariaDB on both the localhost and
asec-cmdenc-dev2.jpl.nasa.gov hostnames, and *can* use TLS to connect, but is
NOT REQUIRED to.  TLS access requires the use of the full hostname, since the
server certificate will not validate for 'localhost'.

To connect using 2-way TLS, with any certificate:

To connect as testuser2 with 2-way TLS, using the local-test-cert.pem
certificate: mysql -u testuser2 -h asec-cmdenc-dev2.jpl.nasa.gov
--ssl-ca=/etc/pki/tls/certs/ammos-ca-bundle.crt --ssl-verify-server-cert
--ssl-cert=/etc/pki/tls/certs/local-test-cert.pem
--ssl-key=/etc/pki/tls/private/local-test-key.pem Password: mTLS does not
require a password.
 **/
...
--ssl-ca=/etc/pki/tls/certs/ammos-ca-bundle.crt --ssl-verify-server-cert
--ssl-cert=/etc/pki/tls/certs/local-test-cert.pem
--ssl-key=/etc/pki/tls/private/local-test-key.pem Password: 2 way TLS or mTLS
does not require a password. However using the MySQL's C API*/
UTEST(MARIA_DB_CONNECTION_TESTS, TLS_TEST) {
  ...
  char *password =
      "l0ngp@ssWord"; // replace with actual password or test will fail.
  ...
  status = Crypto_Config_MariaDB(mysql_hostname, mysql_database, mysql_port,
                                 CRYPTO_TRUE, verify_server, ssl_ca, ssl_capath,
                                 ssl_cert, ssl_key, client_key_password,
  ...
  status = 0;
  // close the connection to avoid a duplicate connection error when running the
  // test multiple times.
  sa_if->sa_close();
  /*connection input parameters.
   Note: username, pass, and paths may differ on your system*/
...
int32_t Crypto_Init_TC_Unit_Test_For_DB(void) {
  ...
  Crypto_Config_CryptoLib(
      KEY_TYPE_INTERNAL, MC_TYPE_INTERNAL, SA_TYPE_MARIADB,
