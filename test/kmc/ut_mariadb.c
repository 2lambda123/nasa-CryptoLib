...
//     ../../src/sa/sa_mariadb_sql/empty_sadb.sql"); system("mysql
//     --host=localhost -uroot -pitc123! <
...
//     status = Crypto_Config_MariaDB(mysql_hostname, mysql_database,
//     mysql_port, CRYPTO_FALSE, verify_server, ssl_ca,
//                                 ssl_capath, ssl_cert, ssl_key,
//                                 client_key_password, mysql_username,
//                                 mysql_password);
//     ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);

//     Crypto_Config_CryptoLib(KEY_TYPE_INTERNAL, MC_TYPE_DISABLED,
//     SA_TYPE_MARIADB, CRYPTOGRAPHY_TYPE_LIBGCRYPT,
...
//     status = sa_if->sa_get_from_spi(2, &test_association);
//     ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
//     ASSERT_EQ(test_association->iv[test_association->iv_len - 1], 2);  //
//     Verify that IV incremented.
...
//     char* raw_tc_sdls_ping_h =
//     "20030415000080d2c70008197f0b00310000b1fe3128"; char* raw_tc_sdls_ping_b
//     = NULL; int raw_tc_sdls_ping_len = 0; SaInterface sa_if =
//     get_sa_interface_mariadb(); hex_conversion(raw_tc_sdls_ping_h,
//     &raw_tc_sdls_ping_b, &raw_tc_sdls_ping_len);
...
//  data, uint8_t* key, uint8_t* iv, uint8_t* header, uint8_t* *bitmask,
//  uint8_t** expected, long* expected_length) function.
//  **/
// UTEST(MARIA_DB, AUTH_DECRYPTION_TEST)
// {
//     char* dec_test_h =
//     "20030433000000030000000000000000000000014ED87188D42B3F36130F355E83F3DE9C5E8F716321145159B41144E5514EBBEA";
...
//     Crypto_TC_ProcessSecurity(dec_test_b, &dec_test_len,
//     tc_sdls_processed_frame); for (int i = 0; i <
//     tc_sdls_processed_frame->tc_pdu_len; i++)
//     {
...
//     free(tc_sdls_processed_frame->tc_sec_trailer.mac); // TODO:  Is there a
//     method to free all of this? free(tc_sdls_processed_frame);
// }
...
//     char* raw_tc_sdls_ping_h =
//     "20030815000080d2c70008197f0b00310000b1fe3128"; char* raw_tc_sdls_ping_b
//     = NULL;
...
//     SaInterface sa_if = get_sa_interface_mariadb();
//     hex_conversion(raw_tc_sdls_ping_h, &raw_tc_sdls_ping_b,
//     &raw_tc_sdls_ping_len); hex_conversion(new_iv_h, &new_iv_b, &new_iv_len);
...
//         *(test_association->iv + i)); ASSERT_EQ(expected_iv_b[i],
...
// /**
//  * @brief Unit Test: Nominal Authorized Encryption With Partial ARSN
//  Rollover, increment static ARSN
//  **/
...
//     char* raw_tc_sdls_ping_h =
//     "20030C15000080d2c70008197f0b00310000b1fe3128"; char* raw_tc_sdls_ping_b
//     = NULL;
...
//     hex_conversion(raw_tc_sdls_ping_h, &raw_tc_sdls_ping_b,
//     &raw_tc_sdls_ping_len); hex_conversion(new_arsn_h, &new_arsn_b,
//     &new_arsn_len); hex_conversion(expected_arsn_h, &expected_arsn_b,
//     &expected_arsn_len); uint8_t* ptr_enc_frame = NULL; uint16_t
//     enc_frame_len = 0;
...
//     {
//         printf("[%d] Truth: %02x, Actual: %02x\n", i, expected_arsn_b[i],
//         *(test_association->arsn + i)); ASSERT_EQ(expected_arsn_b[i],
...
//  * Bad Space Craft ID.  This should pass the flawed .dat file, and return
...
//     "20010015000080d2c70008197f0b00310000b1fe3128"; char*
//     raw_tc_sdls_ping_bad_scid_b = NULL; int raw_tc_sdls_ping_bad_scid_len =
//     0;
...
//     int32_t return_val = Crypto_TC_ApplySecurity((uint8_t*
//     )raw_tc_sdls_ping_bad_scid_b, raw_tc_sdls_ping_bad_scid_len,
//                                                  &ptr_enc_frame,
//                                                  &enc_frame_len);
