...
/*
 *  Simple crypto security program that reads files into memory and calls the
 * appropriate Crypto* function on the data.
 */
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3 || argc % 2 == 0) {
    fprintf(stderr,
            ...
            "specify as many [<tc_a|tm_a|aos_a|tc_p|tm_p|aos_p> <filename>] "
            "pairs as necessary to complete your "
            "crypto sequence test. Each file will be loaded and processed in "
            "sequence. \n"
            "<tc_a|tm_a|aos_a> : Apply TeleCommand (tc_a) | Telemetry (tm_a) | "
            "Advanced Orbiting Systems (aos_a) "
            "Security T\n"
            "<tc_p|tm_p|aos_p> : Process TeleCommand (tc_p) | Telemetry (tm_p) "
            "| Advanced Orbiting Systems (aos_p) "
            "Security T\n"
  ...
  while (arg_index != argc - 1) {
    ...
    if (strcmp(security_type, "tc_a") == 0) {
      Crypto_TC_ApplySecurity((const uint8_t *)buffer,
                              (const uint16_t)buffer_size_i, &ptr_enc_frame,
                              &enc_frame_len);
      ...
      Crypto_TC_ProcessSecurity((uint8_t *)buffer, (int *)&buffer_size_i,
