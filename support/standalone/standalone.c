...
int32_t crypto_standalone_check_number_arguments(int actual, int expected) {
  ...
  if (actual != expected) {
    ...
    printf("Invalid command format or number of arguments, type 'help' for "
           "more info\n");
  }
  return status;
}
...
int32_t crypto_standalone_process_command(int32_t cc, int32_t num_tokens,
  ...
  case CRYPTO_CMD_NOOP:
    if (crypto_standalone_check_number_arguments(num_tokens, 0) ==
        CRYPTO_LIB_SUCCESS) {
  ...
  case CRYPTO_CMD_VCID:
    if (crypto_standalone_check_number_arguments(num_tokens, 1) ==
        CRYPTO_LIB_SUCCESS) {
      ...
      if (vcid < 64) {
        ...
        if ((test_association->sa_state == SA_OPERATIONAL) &&
          ...
          printf("Error - virtual channel (VCID) %d is invalid! Sticking with "
                 "prior vcid %d \n",
                 vcid, tc_vcid);
        }
      } else {
        printf("Error - virtual channl (VCID) %d must be less than 64! "
               "Sticking with prior vcid %d \n",
               vcid, tc_vcid);
      }
    }
    break;
...
int32_t crypto_standalone_udp_init(udp_info_t *sock, int32_t port,
  ...
  /* Bind */
  if (bind_sock > 0) {
    status = bind(sock->sockfd, (struct sockaddr *)&sock->saddr,
                  sizeof(sock->saddr));
    if (status != 0) {
...
void crypto_standalone_tc_frame(uint8_t *in_data, uint16_t in_length,
  ...
  *out_length = (uint16_t)CRYPTO_STANDALONE_FRAMING_TC_DATA_LEN + 6;
  ...
  out_data[2] = ((tc_vcid << 2) & 0xFC) |
                (((uint16_t)CRYPTO_STANDALONE_FRAMING_TC_DATA_LEN >> 8) & 0x03);
...
void *crypto_standalone_tc_apply(void *socks) {
  ...
  while (keepRunning == CRYPTO_LIB_SUCCESS) {
    ...
    status = recvfrom(tc_read_sock->sockfd, tc_apply_in, sizeof(tc_apply_in), 0,
                      (struct sockaddr *)&tc_read_sock->ip_address,
    ...
    if (status != -1) {
      ...
      /* Frame */
#ifdef CRYPTO_STANDALONE_HANDLE_FRAMING
      crypto_standalone_tc_frame(tc_apply_in, tc_in_len, tc_framed,
                                 &tc_out_len);
      ...
      /* Process */
      status = Crypto_TC_ApplySecurity(tc_apply_in, tc_in_len, &tc_out_ptr,
                                       &tc_out_len);
      if (status == CRYPTO_LIB_SUCCESS) {
        if (tc_debug == 1) {
          printf("crypto_standalone_tc_apply - status = %d, encrypted[%d]: 0x",
                 status, tc_out_len);
        ...
        status = sendto(tc_write_sock->sockfd, tc_out_ptr, tc_out_len, 0,
                        (struct sockaddr *)&tc_write_sock->saddr,
                        sizeof(tc_write_sock->saddr));
...
void crypto_standalone_tm_frame(uint8_t *in_data, uint16_t in_length,
                                uint8_t *out_data, uint16_t *out_length,
                                uint16_t spi) {
  SaInterface sa_if = get_sa_interface_inmemory();
  ...
  uint8_t header_length = 6 + 2 + sa_ptr->shivf_len + sa_ptr->shplf_len +
                          sa_ptr->shsnf_len + 40; // TODO: Why +40?
  ...
  *out_length = (uint16_t)in_length - header_length - trailer_length;
  ...
  /* TM Header */
  memcpy(out_data, &in_data[header_length],
         in_length - header_length - trailer_length);
}

void crypto_standalone_tm_debug_recv(int32_t status, int tm_process_len,
                                     uint8_t *tm_process_in) {
  if (tm_debug == 1) {
...
void crypto_standalone_spp_telem_or_idle(int32_t *status_p, uint8_t *tm_ptr,
                                         uint16_t *spp_len_p,
                                         udp_interface_t *tm_socks,
  ...
  if ((tm_ptr[0] == 0x08) || ((tm_ptr[0] == 0x03) && tm_ptr[1] == 0xff)) {
    ...
    if (tm_ptr[0] == 0x08) {
      status = sendto(tm_write_sock->sockfd, tm_ptr, spp_len, 0,
                      (struct sockaddr *)&tm_write_sock->saddr,
    ...
    printf("crypto_standalone_tm_process - SPP loop error, expected idle "
           "packet or frame! tm_ptr = 0x%02x%02x \n",
           tm_ptr[0], tm_ptr[1]);
    tm_process_len = 0;
  }
  *status_p = status;
...
void *crypto_standalone_tm_process(void *socks) {
  ...
  uint8_t tm_process_in[TM_CADU_SIZE]; // Accounts for ASM automatically based
                                       // on #def
  ...
  while (keepRunning == CRYPTO_LIB_SUCCESS) {
    /* Receive */
    status =
        recvfrom(tm_read_sock->sockfd, tm_process_in, sizeof(tm_process_in), 0,
                 (struct sockaddr *)&tm_read_sock->ip_address,
                 (socklen_t *)&sockaddr_size);
    if (status != -1) {
      ...
      status = Crypto_TM_ProcessSecurity(tm_process_in + 4,
                                         (const uint16_t)tm_process_len - 4,
                                         &tm_ptr, &tm_out_len);
#else
      if (tm_debug == 1) {
        printf("Processing frame without ASM...\n");
      }
      status = Crypto_TM_ProcessSecurity(
          tm_process_in, (const uint16_t)tm_process_len, &tm_ptr, &tm_out_len);
#endif
      if (status == CRYPTO_LIB_SUCCESS) {
        if (tm_debug == 1) {
          if (((tm_ptr[4] & 0x0F) == 0x0F) && (tm_ptr[5] == 0xFE)) {
            ...
            printf("crypto_standalone_tm_process: 1 - status = %d, "
                   "decrypted[%d]: 0x",
                   status, tm_out_len);
        ...
        // crypto_standalone_tm_frame(tm_process_in+4, tm_process_len-4,
        // tm_framed, &tm_framed_len, spi);
        crypto_standalone_tm_frame(tm_ptr, tm_out_len, tm_framed,
                                   &tm_framed_len, spi);
#else
        uint16_t spi = (0xFFFF & tm_process_in[7]) | tm_process_in[8];
        crypto_standalone_tm_frame(tm_process_in, tm_process_len, tm_framed,
                                   &tm_framed_len, spi);
#endif
        ...
        {
          printf("crypto_standalone_tm_process: 2 - beginning after first "
                 "header pointer - deframed[%d]: 0x",
                 tm_process_len);
        ...
        while (tm_process_len > 5) {
          ...
          crypto_standalone_spp_telem_or_idle(&status, tm_ptr, &spp_len,
                                              tm_socks, &tm_process_len);
        }
      } else {
...
int main(int argc, char *argv[]) {
  ...
  if (keepRunning == CRYPTO_LIB_SUCCESS) {
    ...
    if (status != CRYPTO_LIB_SUCCESS) {
      printf(
          "crypto_standalone_udp_init tc_apply.read failed with status %d \n",
          status);
      keepRunning = CRYPTO_LIB_ERROR;
    } else {
      status =
          crypto_standalone_udp_init(&tc_apply.write, TC_APPLY_FWD_PORT, 0);
      if (status != CRYPTO_LIB_SUCCESS) {
        printf("crypto_standalone_udp_init tc_apply.write failed with status "
               "%d \n",
               status);
        keepRunning = CRYPTO_LIB_ERROR;
      }
  ...
  if (keepRunning == CRYPTO_LIB_SUCCESS) {
    ...
    if (status != CRYPTO_LIB_SUCCESS) {
      printf(
          "crypto_standalone_udp_init tm_apply.read failed with status %d \n",
          status);
      keepRunning = CRYPTO_LIB_ERROR;
    } else {
      status =
          crypto_standalone_udp_init(&tm_process.write, TM_PROCESS_FWD_PORT, 0);
      if (status != CRYPTO_LIB_SUCCESS) {
        printf("crypto_standalone_udp_init tc_apply.write failed with status "
               "%d \n",
               status);
  ...
  if (keepRunning == CRYPTO_LIB_SUCCESS) {
    printf("  TC Apply \n");
    printf("    Read, UDP - %s : %d \n", tc_apply.read.ip_address,
           tc_apply.read.port);
    printf("    Write, UDP - %s : %d \n", tc_apply.write.ip_address,
           tc_apply.write.port);
    printf("  TM Process \n");
    printf("    Read, UDP - %s : %d \n", tm_process.read.ip_address,
           tm_process.read.port);
    printf("    Write, UDP - %s : %d \n", tm_process.write.ip_address,
           tm_process.write.port);
    printf("\n");
    ...
    if (status < 0) {
      ...
      status = pthread_create(&tm_process_thread, NULL,
                              *crypto_standalone_tm_process, &tm_process);
      if (status < 0) {
  ...
  while (keepRunning == CRYPTO_LIB_SUCCESS) {
    ...
    while ((num_input_tokens < CRYPTO_MAX_INPUT_TOKENS) &&
           (token_ptr != NULL)) {
