...
** Definitions
*/
typedef struct {      // Global Virtual Channel ID / Global MAP ID
...
typedef struct {
  ...
  uint16_t ekid; // Encryption Key ID  (Used with numerically indexed keystores,
                 // EG inmemory keyring)
  uint16_t akid; // Authentication Key ID
  char
      ek_ref[REF_SIZE]; // Encryption Key Reference (Used with string-referenced
                        // keystores,EG-PKCS12 keystores, KMC crypto)
  char ak_ref[REF_SIZE]; // Authentication Key Reference (Used with
                         // string-referenced keystores,EG-PKCS12 keystores, KMC
                         // crypto)
  uint8_t sa_state : 2;
  ...
  uint8_t abm[ABM_SIZE];   // Authentication Bit Mask (Primary Hdr. through
                           // Security Hdr.)
...
*/
// typedef struct __attribute__ ((packed)) //__attribute__ ((packed)) is not
// easily supported in CFFI python. Only add when CFFI properly supports packed
// & nonpacked structs.
typedef struct {
...
typedef struct {
  ...
  // 0 = Payload is either idle data or octet synchronized forward-ordered
  // packets 1 = Data is a virtual channel access data unit
  ...
  // Sync Flag 0 = Contains position of the first byte of the first packet in
  // the data field Sync Flag 1 = undefined uint8_t	tfshvn	:2;
  // // Transfer Frame Secondary Header Version Number - shall be 00 uint8_t
  // tfshlen	:6;			// TFSH Length (max 64 Bytes)
...
#define TM_MIN_SIZE                                                            \
  (TM_FRAME_PRIMARYHEADER_SIZE + TM_FRAME_SECHEADER_SIZE +                     \
...
typedef struct {
  ...
  uint16_t fhp : 11; // First Header Pointer
  // Sync Flag 0 = Contains position of the first byte of the first packet in
  // the data field Sync Flag 1 = undefined
...
#define AOS_MIN_SIZE                                                           \
  (AOS_FRAME_PRIMARYHEADER_SIZE + AOS_FRAME_SECHEADER_SIZE +                   \
   AOS_FRAME_SECTRAILER_SIZE + AOS_FRAME_OCF_SIZE)
