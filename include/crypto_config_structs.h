...
from, or in any way connected with the software or its documentation. Whether or
not based upon warranty, contract, tort or otherwise, and whether or not loss
...
// main config enums
typedef enum { UNITIALIZED = 0, INITIALIZED } InitStatus;
typedef enum {
...
****************************************/
typedef enum { IV_INTERNAL, IV_CRYPTO_MODULE } IvType;
typedef enum {
  TC_NO_FECF,
  TC_HAS_FECF,
  TM_NO_FECF,
...
typedef enum { AOS_FHEC_NA = 0, AOS_NO_FHEC, AOS_HAS_FHEC } AosFhecPresent;
typedef enum { AOS_IZ_NA, AOS_NO_IZ, AOS_HAS_IZ } AosInsertZonePresent;
typedef enum {
  TC_CHECK_FECF_FALSE,
  TC_CHECK_FECF_TRUE,
  TM_CHECK_FECF_FALSE,
...
typedef enum { CAM_ENABLED_FALSE, CAM_ENABLED_TRUE } CamEnabledBool;

typedef enum {
  CAM_LOGIN_NONE,       // Using already populated cam_cookie_file
  CAM_LOGIN_KERBEROS,   // Using already logged-in Kerberos to generate CAM
                        // cookies
...
typedef struct {
  ...
  CreateFecfBool crypto_create_fecf; // Whether or not CryptoLib is expected to
                                     // calculate TC FECFs and return
  // payloads with the FECF
  TcProcessSdlsPdus process_sdls_pdus; // Config to process SDLS extended
                                       // procedure PDUs in CryptoLib
  TcPusHdrPresent has_pus_hdr;
  TcIgnoreSaState
      ignore_sa_state; // TODO - add logic that uses this configuration
  ...
  uint8_t crypto_increment_nontransmitted_iv; // Whether or not CryptoLib
                                              // increments the non-transmitted
                                              // portion of the IV field
} CryptoConfig_t;
...
#define CRYPTOGRAPHY_KMC_CRYPTO_SERVICE_CONFIG_SIZE                            \
  (sizeof(CryptographyKmcCryptoServiceConfig_t))
