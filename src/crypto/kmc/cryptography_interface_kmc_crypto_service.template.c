...
static int32_t cryptography_encrypt(uint8_t *data_out, size_t len_data_out,
                                    ...
                                    SecurityAssociation_t *sa_ptr, uint8_t *iv,
                                    uint32_t iv_len, uint8_t *ecs,
                                    uint8_t padding, char *cam_cookies);
static int32_t cryptography_decrypt(uint8_t *data_out, size_t len_data_out,
                                    uint8_t *data_in, size_t len_data_in,
                                    uint8_t *key, uint32_t len_key,
                                    SecurityAssociation_t *sa_ptr, uint8_t *iv,
                                    uint32_t iv_len, uint8_t *ecs, uint8_t *acs,
                                    char *cam_cookies);
static int32_t
cryptography_authenticate(uint8_t *data_out, size_t len_data_out,
                          uint8_t *data_in, size_t len_data_in, uint8_t *key,
                          uint32_t len_key, SecurityAssociation_t *sa_ptr,
                          uint8_t *iv, uint32_t iv_len, uint8_t *mac,
                          uint32_t mac_size, uint8_t *aad, uint32_t aad_len,
                          uint8_t ecs, uint8_t acs, char *cam_cookies);
static int32_t cryptography_validate_authentication(
    uint8_t *data_out, size_t len_data_out, const uint8_t *data_in,
    const size_t len_data_in, uint8_t *key, uint32_t len_key,
    SecurityAssociation_t *sa_ptr, const uint8_t *iv, uint32_t iv_len,
    const uint8_t *mac, uint32_t mac_size, const uint8_t *aad, uint32_t aad_len,
    uint8_t ecs, uint8_t acs, char *cam_cookies);
static int32_t cryptography_aead_encrypt(
    uint8_t *data_out, size_t len_data_out, uint8_t *data_in,
    size_t len_data_in, uint8_t *key, uint32_t len_key,
    SecurityAssociation_t *sa_ptr, uint8_t *iv, uint32_t iv_len, uint8_t *mac,
    uint32_t mac_size, uint8_t *aad, uint32_t aad_len, uint8_t encrypt_bool,
    uint8_t authenticate_bool, uint8_t aad_bool, uint8_t *ecs, uint8_t *acs,
    char *cam_cookies);
static int32_t cryptography_aead_decrypt(
    uint8_t *data_out, size_t len_data_out, uint8_t *data_in,
    size_t len_data_in, uint8_t *key, uint32_t len_key,
    SecurityAssociation_t *sa_ptr, uint8_t *iv, uint32_t iv_len, uint8_t *mac,
    uint32_t mac_size, uint8_t *aad, uint32_t aad_len, uint8_t decrypt_bool,
    uint8_t authenticate_bool, uint8_t aad_bool, uint8_t *ecs, uint8_t *acs,
    char *cam_cookies);
...
static int32_t get_auth_algorithm_from_acs(uint8_t acs_enum,
                                           const char **algo_ptr);
static int32_t get_cam_sso_token(void);
static int32_t initialize_kerberos_keytab_file_login(void);
static int32_t curl_perform_with_cam_retries(CURL *curl_handle,
                                             memory_write *chunk_write,
                                             memory_read *chunk_read);
...
static const char *encrypt_endpoint =
    "encrypt?keyRef=%s&transformation=%s&iv=%s";
static const char *encrypt_endpoint_null_iv =
    "encrypt?keyRef=%s&transformation=%s";
static const char *encrypt_offset_endpoint =
    "encrypt?keyRef=%s&transformation=%s&iv=%s&encryptOffset=%s&macLength=%s";
static const char *encrypt_offset_endpoint_null_iv =
    "encrypt?keyRef=%s&transformation=%s&encryptOffset=%s&macLength=%s";
static const char *decrypt_endpoint =
    "decrypt?metadata=keyLength:%s,keyRef:%s,cipherTransformation:%s,"
    "initialVector:%s,cryptoAlgorithm:%s,metadataType:EncryptionMetadata";
static const char *decrypt_offset_endpoint =
    "decrypt?metadata=keyLength:%s,keyRef:%s,cipherTransformation:%s,"
    "initialVector:%s,cryptoAlgorithm:%s,macLength:%s,metadataType:"
    "EncryptionMetadata,encryptOffset:%s";
static const char *icv_create_endpoint = "icv-create?keyRef=%s";
static const char *icv_verify_endpoint =
    "icv-verify?metadata=integrityCheckValue:%s,keyRef:%s,cryptoAlgorithm:%s,"
    "macLength:%s,metadataType:IntegrityCheckMetadata";

// CAM Security Endpoints
static const char *cam_kerberos_uri =
    "%s/cam-api/ssoToken?loginMethod=kerberos";
...
static int32_t cryptography_config(void) {
  ...
  // Error out if Crypto_Config_Kmc_Crypto_Service(...) function was not called
  // before intializing library.
  if (cryptography_kmc_crypto_config == NULL) {
    fprintf(stderr, "You must configure the KMC Crypto Service before starting "
                    "the interface!\n");
  ...
  if (curl) {
    ...
    char *port_str = int_to_str(cryptography_kmc_crypto_config->kmc_crypto_port,
                                &port_str_len);

    // Form Root URI
    // len(protocol)+len(://)+len(hostname)+ len(:) + len(port_str) + len(/) +
    // len(app_uri) + strlen('\0')
    uint32_t len_root_uri =
        strlen(cryptography_kmc_crypto_config->protocol) + 3 + // "://"
    ...
    snprintf(kmc_root_uri, len_root_uri, "%s://%s:%s/%s/",
    ...
    free(port_str);
    // KMC Crypto Service status check is impossible in certain CAM configs,
    // commenting it out.
    //  Also, when this library is started up (EG by SDLS service), there's no
    //  guarantee the Crypto Service is available at config time.
    // char* status_uri = (char*)
    // malloc(strlen(kmc_root_uri)+strlen(status_endpoint) + 1); status_uri[0] =
    // '\0'; strcat(status_uri, kmc_root_uri); strcat(status_uri,
    // status_endpoint);
#ifdef DEBUG
    ...
    // if(res != CURLE_OK) // This is not return code, this is successful
    // response!
    //{
    ...
    // if(chunk->response == NULL) // No response, possibly because service is
    // CAM secured.
    //{
    //     status = CRYPTOGRAHPY_KMC_CRYPTO_SERVICE_EMPTY_RESPONSE;
    //     fprintf(stderr, "curl_easy_perform() unexpected empty response:
    //     \n%s\n",
    //             "Empty Crypto Service response can be caused by CAM security,
    //             is CAM configured?");
    //     free(status_uri);
    //     return status;
    // }
...
static int32_t cryptography_init(void) {
  ...
  http_headers_list = curl_slist_append(
      http_headers_list, "Content-Type: application/octet-stream");
  // http_headers_list = curl_slist_append(http_headers_list, "Accept:
  // application/json"); curl_slist_append(http_headers_list, "Content-Type:
  // application/json"); http_headers_list =
  // curl_slist_append(http_headers_list, "charset: utf-8");
...
static int32_t cryptography_encrypt(uint8_t *data_out, size_t len_data_out,
  ...
  int len_encrypt_endpoint = strlen(encrypt_endpoint) + strlen(sa_ptr->ek_ref) +
                             strlen(iv_base64) + strlen(AES_CBC_TRANSFORMATION);
  char *encrypt_endpoint_final = (char *)malloc(len_encrypt_endpoint);
  if (iv == NULL) {
    snprintf(encrypt_endpoint_final, len_encrypt_endpoint,
             encrypt_endpoint_null_iv, sa_ptr->ek_ref, AES_CBC_TRANSFORMATION);
  } else {
    snprintf(encrypt_endpoint_final, len_encrypt_endpoint, encrypt_endpoint,
             sa_ptr->ek_ref, AES_CBC_TRANSFORMATION, iv_base64);
  }
  ...
  jsmn_init(&p);
  int parse_result = jsmn_parse(
      &p, chunk_write->response, strlen(chunk_write->response), t,
  ...
  for (json_idx = 1; json_idx < parse_result; json_idx++) {
    if (jsoneq(chunk_write->response, &t[json_idx], "metadata") == 0) {
      ...
      char *token;
      char temp_buff[256];
      for (line = strtok(ciphertext_IV_base64, ","); line != NULL;
           line = strtok(line + strlen(line) + 1, ",")) {
        strncpy(temp_buff, line, sizeof(temp_buff));

        for (token = strtok(temp_buff, ":"); token != NULL;
          ...
          if (strcmp(token, "initialVector") == 0) {
            ...
            base64urlDecode(ciphertext_token_base64, cipher_text_token_len,
                            iv_decoded, &iv_decoded_len);
#ifdef DEBUG
            ...
            if (iv == NULL) {
              memcpy(data_out - sa_ptr->shsnf_len - sa_ptr->shivf_len -
                         sa_ptr->shplf_len,
                     iv_decoded, iv_decoded_len);
            }
    ...
    if (jsoneq(chunk_write->response, &t[json_idx], "base64ciphertext") == 0) {
      ...
      memcpy(ciphertext_base64, chunk_write->response + t[json_idx + 1].start,
             len_ciphertext);
    ...
    if (jsoneq(chunk_write->response, &t[json_idx], "httpCode") == 0) {
      ...
      memcpy(http_code_str, chunk_write->response + t[json_idx + 1].start,
             len_httpcode);
  ...
  base64Decode(ciphertext_base64, strlen(ciphertext_base64), ciphertext_decoded,
...
static int32_t cryptography_decrypt(uint8_t *data_out, size_t len_data_out,
  ...
  // TODO -- Parse the key length from the keyInfo endpoint of the Crypto
  // Service!
  ...
  int len_decrypt_endpoint =
      strlen(decrypt_endpoint) + key_len_in_bits_str_len +
      strlen(sa_ptr->ek_ref) + strlen(iv_base64) +
      strlen(AES_CBC_TRANSFORMATION) + strlen(AES_CRYPTO_ALGORITHM);
  char *decrypt_endpoint_final = (char *)malloc(len_decrypt_endpoint);

  snprintf(decrypt_endpoint_final, len_decrypt_endpoint, decrypt_endpoint,
           key_len_in_bits_str, sa_ptr->ek_ref, AES_CBC_TRANSFORMATION,
           iv_base64, AES_CRYPTO_ALGORITHM);
  ...
  for (json_idx = 1; json_idx < parse_result; json_idx++) {
    ...
    if (jsoneq(chunk_write->response, &t[json_idx], "base64cleartext") == 0) {
      ...
      memcpy(cleartext_base64, chunk_write->response + t[json_idx + 1].start,
             len_cleartext);
  ...
  base64Decode(cleartext_base64, strlen(cleartext_base64), cleartext_decoded,
...
cryptography_authenticate(uint8_t *data_out, size_t len_data_out,
  ...
  // Need to copy the data over, since authentication won't change/move the data
  // directly
  if (data_out != NULL) {
  ...
  snprintf(auth_endpoint_final, len_auth_endpoint, icv_create_endpoint,
           sa_ptr->ak_ref);
  ...
  for (json_idx = 1; json_idx < parse_result; json_idx++) {
    if (jsoneq(chunk_write->response, &t[json_idx], "metadata") == 0) {
      ...
      memcpy(metadata, chunk_write->response + t[json_idx + 1].start,
             len_metadata);
  ...
  uint8_t *icv_decoded =
      calloc(1, B64DECODE_OUT_SAFESIZE(strlen(icv_base64)) + 1);
  size_t icv_decoded_len = 0;
  base64urlDecode(icv_base64, strlen(icv_base64), icv_decoded,
                  &icv_decoded_len);
#ifdef DEBUG
  printf("Mac size: %d\n", mac_size);
...
static int32_t cryptography_validate_authentication(
  ...
  // Prepare the Authentication Endpoint URI for KMC Crypto Service
  int len_auth_endpoint = strlen(icv_verify_endpoint) + strlen(mac_base64) +
                          strlen(sa_ptr->ak_ref) + strlen(auth_algorithm) +
                          mac_size_str_len;
  char *auth_endpoint_final = (char *)malloc(len_auth_endpoint);
  snprintf(auth_endpoint_final, len_auth_endpoint, icv_verify_endpoint,
           mac_base64, sa_ptr->ak_ref, auth_algorithm, mac_size_str);
  free(mac_size_str);
  char *auth_uri = (char *)malloc(strlen(kmc_root_uri) + len_auth_endpoint);
  auth_uri[0] = '\0';
  strcat(auth_uri, kmc_root_uri);
  ...
  for (json_idx = 1; json_idx < parse_result; json_idx++) {
    ...
    if (jsoneq(chunk_write->response, &t[json_idx], "result") == 0) {
      ...
      memcpy(result_str, chunk_write->response + t[json_idx + 1].start,
             len_result);
...
#endif
      if (strcmp(result_str, "true") !=
          0) // KMC crypto service returns true string if ICV check succeeds.
      {
...
static int32_t cryptography_aead_encrypt(
  ...
  if (aad_bool == CRYPTO_TRUE) {
    // Determine length of aad offset string and convert to string for use in
    // URL
    ...
    int len_encrypt_endpoint = strlen(encrypt_offset_endpoint) +
                               strlen(sa_ptr->ek_ref) + strlen(iv_base64) +
                               strlen(AES_GCM_TRANSFORMATION) +
                               aad_offset_str_len + mac_size_str_len;
    char *encrypt_endpoint_final = (char *)malloc(len_encrypt_endpoint);
    if (iv != NULL) {

      snprintf(encrypt_endpoint_final, len_encrypt_endpoint,
               encrypt_offset_endpoint, sa_ptr->ek_ref, AES_GCM_TRANSFORMATION,
               iv_base64, aad_offset_str, mac_size_str);
    } else {
      //"encrypt?keyRef=%s&transformation=%s&encryptOffset=%s&macLength=%s";
      snprintf(encrypt_endpoint_final, len_encrypt_endpoint,
               encrypt_offset_endpoint_null_iv, sa_ptr->ek_ref,
    ...
    if (encrypt_bool ==
        CRYPTO_FALSE) // Not encrypting data, only passing in AAD for TAG.
    {
  ...
  {
    int len_encrypt_endpoint = strlen(encrypt_endpoint) +
                               strlen(sa_ptr->ek_ref) + strlen(iv_base64) +
                               strlen(AES_GCM_TRANSFORMATION);
    char *encrypt_endpoint_final = (char *)malloc(len_encrypt_endpoint);
    if (iv != NULL) {
      snprintf(encrypt_endpoint_final, len_encrypt_endpoint, encrypt_endpoint,
               sa_ptr->ek_ref, AES_GCM_TRANSFORMATION, iv_base64);
    } else {
      snprintf(encrypt_endpoint_final, len_encrypt_endpoint,
               encrypt_endpoint_null_iv, sa_ptr->ek_ref,
  ...
  uint8_t *ciphertext_decoded =
      malloc((len_data_out + mac_size + aad_len) * 2 + 1);
  size_t ciphertext_decoded_len = 0;
  base64Decode(ciphertext_base64, strlen(ciphertext_base64), ciphertext_decoded,
               &ciphertext_decoded_len);
#ifdef DEBUG
...
static int32_t cryptography_aead_decrypt(
  ...
  if (aad_bool == CRYPTO_TRUE) {
    ...
    int len_decrypt_endpoint =
        strlen(decrypt_offset_endpoint) + key_len_in_bits_str_len +
        strlen(sa_ptr->ek_ref) + strlen(iv_base64) +
        strlen(AES_GCM_TRANSFORMATION) + strlen(AES_CRYPTO_ALGORITHM) +
        mac_size_str_len + aad_offset_str_len;
    char *decrypt_endpoint_final = (char *)malloc(len_decrypt_endpoint);

    snprintf(decrypt_endpoint_final, len_decrypt_endpoint,
             decrypt_offset_endpoint, key_len_in_bits_str, sa_ptr->ek_ref,
             AES_GCM_TRANSFORMATION, iv_base64, AES_CRYPTO_ALGORITHM,
             mac_size_str, aad_offset_str);
    ...
    if (decrypt_bool == CRYPTO_FALSE) // Not decrypting data, only passing in
                                      // AAD for TAG validation.
    {
  ...
  } else // No AAD - just prepare the endpoint URI string
  {
    ...
    snprintf(decrypt_endpoint_final, len_decrypt_endpoint, decrypt_endpoint,
             key_len_in_bits_str, sa_ptr->ek_ref, AES_GCM_TRANSFORMATION,
             iv_base64, AES_CRYPTO_ALGORITHM);
...
static int32_t get_auth_algorithm_from_acs(uint8_t acs_enum,
                                           const char **algo_ptr) {
  int32_t status =
      CRYPTO_LIB_ERR_UNSUPPORTED_ACS; // All valid algo enums will be positive
...
static int32_t configure_curl_connect_opts(CURL *curl_handle,
                                           char *cam_cookies) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  ...
  // curl_easy_setopt(curl_handle, CURLOPT_PROTOCOLS,CURLPROTO_HTTPS); // use
  // default CURLPROTO_ALL
#ifdef DEBUG
...
#endif
  curl_easy_setopt(curl_handle, CURLOPT_PORT,
                   cryptography_kmc_crypto_config->kmc_crypto_port);
  curl_easy_setopt(curl_handle, CURLOPT_SSLCERT,
                   cryptography_kmc_crypto_config->mtls_client_cert_path);
  curl_easy_setopt(curl_handle, CURLOPT_SSLKEY,
                   cryptography_kmc_crypto_config->mtls_client_key_path);
  if (cryptography_kmc_crypto_config->mtls_client_cert_type != NULL) {
    curl_easy_setopt(curl_handle, CURLOPT_SSLCERTTYPE,
                     cryptography_kmc_crypto_config->mtls_client_cert_type);
  }
  if (cryptography_kmc_crypto_config->mtls_client_key_pass != NULL) {
    curl_easy_setopt(curl_handle, CURLOPT_KEYPASSWD,
                     cryptography_kmc_crypto_config->mtls_client_key_pass);
  }
  if (cryptography_kmc_crypto_config->mtls_ca_bundle != NULL) {
  ...
  if (cryptography_kmc_crypto_config->mtls_issuer_cert != NULL) {
    curl_easy_setopt(curl_handle, CURLOPT_ISSUERCERT,
                     cryptography_kmc_crypto_config->mtls_issuer_cert);
  }
  if (cryptography_kmc_crypto_config->ignore_ssl_hostname_validation ==
      CRYPTO_TRUE) {
...
static int32_t handle_cam_cookies(CURL *curl_handle, char *cam_cookies) {
  ...
  if (cam_config != NULL) {
    if (cam_config->cam_enabled) {
      if (cam_cookies != NULL) // cam_cookies passed in should ALWAYS take
                               // precedence in what's used
      {
      ...
      if (cam_config->cookie_file_path ==
          NULL) // all auth methods rely on cookie file sets/gets, error if null
      {
...
static int32_t get_cam_sso_token() {
  ...
  if (cryptography_kmc_crypto_config->mtls_ca_bundle != NULL) {
    curl_easy_setopt(curl_cam, CURLOPT_CAINFO,
                     cryptography_kmc_crypto_config->mtls_ca_bundle);
  }
  if (cryptography_kmc_crypto_config->mtls_ca_path != NULL) {
    curl_easy_setopt(curl_cam, CURLOPT_CAPATH,
                     cryptography_kmc_crypto_config->mtls_ca_path);
  }
  if (cryptography_kmc_crypto_config->mtls_issuer_cert != NULL) {
    curl_easy_setopt(curl_cam, CURLOPT_ISSUERCERT,
                     cryptography_kmc_crypto_config->mtls_issuer_cert);
  }
  if (cryptography_kmc_crypto_config->ignore_ssl_hostname_validation ==
      CRYPTO_TRUE) {
  ...
  int len_kerberos_endpoint =
      strlen(cam_kerberos_uri) + strlen(cam_config->access_manager_uri);
  char *kerberos_endpoint_final = (char *)malloc(len_kerberos_endpoint + 1);
  snprintf(kerberos_endpoint_final, len_kerberos_endpoint, cam_kerberos_uri,
           cam_config->access_manager_uri);
...
int32_t curl_response_error_check(CURL *curl_handle, char *response) {
    ...
    if (cam_config != NULL) {
      if (cam_config->cam_enabled &&
          cam_config->login_method ==
              CAM_LOGIN_NONE) // redirect with cam enabled and no auth method
                              // means auth failure
      {
        response_status = CAM_AUTHENTICATION_FAILURE_REDIRECT;
        return response_status;
      } else if (cam_config->cam_enabled &&
                 (cam_config->login_method == CAM_LOGIN_KERBEROS ||
                  cam_config->login_method == CAM_LOGIN_KEYTAB_FILE)) {
        response_status = CAM_AUTHENTICATION_REQUIRED;
        return response_status;
      } else // CAM not enabled, but cam is configured and 302 received --
  ...
  if (response == NULL) // No response, possibly because service is CAM secured.
  {
    ...
    fprintf(stderr, "curl_easy_perform() unexpected empty response: \n%s\n",
            "Empty Crypto Service response can be caused by CAM security, is "
            "CAM configured?");
    return response_status;
  }
...
int32_t curl_perform_with_cam_retries(CURL *curl_handle,
                                      memory_write *chunk_write,
                                      memory_read *chunk_read) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  uint8_t cam_retry = 0;
  while (cam_retry < CAM_MAX_AUTH_RETRIES) {
    ...
    if (res != CURLE_OK) // This is not a response w/return code, this is
                         // something breaking!
    {
      status = CRYPTOGRAHPY_KMC_CRYPTO_SERVICE_GENERIC_FAILURE;
      fprintf(stderr, "curl_easy_perform() failed: %s\n",
    ...
    if (status == CRYPTO_LIB_SUCCESS) // Crypto Service REST call worked! Break
                                      // out of retry loop.
    {
      break;
    } else {
    ...
    if (status == CAM_AUTHENTICATION_REQUIRED) // CAM_AUTHENTICATION_REQUIRED
                                               // code indicates CAM config
                                               // setup for authentication
    {
      ...
      if (status == CAM_KERBEROS_REQUEST_TIME_OUT) {
        ...
        // Re-handle CAM cookie file, when cookie file is regenerated above, the
        // existing curl_handle doesn't recognize it
  ...
  if (status == CRYPTO_LIB_SUCCESS &&
      chunk_write->response ==
          NULL) // no error case detected, but invalid NULL response!
  {
    status = CRYPTOGRAHPY_KMC_CRYPTO_SERVICE_EMPTY_RESPONSE;
  }
  return status;
 ...
 *
 *  This function launches the kinit kerberos utility to initialize credentials
 *for the current user. It may be possible to do this with the KRB5 API, but for
 *now all we are doing is launching the kinit utility. Consider using the KRB5
...
int32_t initialize_kerberos_keytab_file_login(void) {
  ...
  uint32_t len_kinit_shell_command = strlen(kinit_shell_command_base) +
                                     strlen(cam_config->keytab_file_path) +
                                     strlen(cam_config->username);
  char *kinit_shell_command = malloc(len_kinit_shell_command + 1);
  snprintf(kinit_shell_command, len_kinit_shell_command,
           kinit_shell_command_base, cam_config->keytab_file_path,
           cam_config->username);
...
/**
 * @brief Function: cryptography_get_acs_algo. Maps Cryptolib ACS enums to KMC
 *enums It is possible for supported algos to vary between crypto libraries
 * @param algo_enum
 **/
...
/**
 * @brief Function: cryptography_get_ecs_algo. Maps Cryptolib ECS enums to KMC
 *enums It is possible for supported algos to vary between crypto libraries
 * @param algo_enum
 **/
int32_t cryptography_get_ecs_algo(int8_t algo_enum) {
  int32_t algo =
      CRYPTO_LIB_ERR_UNSUPPORTED_ECS; // All valid algo enums will be positive
  switch (algo_enum) {
