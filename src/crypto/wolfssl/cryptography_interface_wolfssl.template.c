...
static int32_t cryptography_authenticate(
  ...
  // Reference:
  // https://www.wolfssl.com/documentation/manuals/wolfssl/group__HMAC.html
...
static int32_t cryptography_encrypt(uint8_t *data_out, size_t len_data_out,
  ...
  case CRYPTO_CIPHER_AES256_GCM:
    ...
    if (status == 0) {
      status = wc_AesGcmEncrypt(&enc, data_out, data_in, len_data_in, iv,
                                iv_len, NULL, 16, NULL, 0);
      if (status == -180) { // Special error case as Wolf will not accept a zero
                            // value for MAC size
  ...
  case CRYPTO_CIPHER_AES256_GCM:
    ...
    if (status == 0) {
      if ((encrypt_bool == CRYPTO_TRUE) && (authenticate_bool == CRYPTO_TRUE)) {
        status = wc_AesGcmEncrypt(&enc, data_out, data_in, len_data_in, iv,
                                  iv_len, mac, mac_size, aad, aad_len);
      } else if (encrypt_bool == CRYPTO_TRUE) {
        status = wc_AesGcmEncrypt(&enc, data_out, data_in, len_data_in, iv,
                                  iv_len, mac, 16, aad, aad_len);
        if (status == -180) { // Special error case as Wolf will not accept a
                              // zero value for MAC size
          status = CRYPTO_LIB_SUCCESS;
        }
      } else if (authenticate_bool == CRYPTO_TRUE) {
        status = wc_AesGcmEncrypt(&enc, data_out, data_in, 0, iv, iv_len, mac,
                                  mac_size, aad, aad_len);
      }
    }
    break;
...
static int32_t cryptography_decrypt(uint8_t *data_out, size_t len_data_out,
  ...
  case CRYPTO_CIPHER_AES256_GCM:
    ...
    if (status == 0) {
      status = wc_AesGcmDecrypt(&dec, data_out, data_in, len_data_in, iv,
                                iv_len, calc_mac, 16, NULL, 0);
      if (status == -180) { // Special error case as Wolf will not accept a zero
                            // value for MAC size
        status = CRYPTO_LIB_SUCCESS;
      }
  ...
  case CRYPTO_CIPHER_AES256_GCM:
    ...
    if (status == 0) {
      if ((decrypt_bool == CRYPTO_TRUE) && (authenticate_bool == CRYPTO_TRUE)) {
        ...
        status = wc_AesGcmDecrypt(&dec, data_out, data_in, len_data_in, iv,
                                  iv_len, mac, mac_size, aad, aad_len);
        // If authentication only, don't decrypt the data. Just pass the data
        // PDU through.
        memcpy(data_out, data_in, len_data_in);
      }
    }
