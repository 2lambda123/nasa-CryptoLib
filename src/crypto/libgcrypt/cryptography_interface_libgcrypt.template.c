...
static int32_t cryptography_authenticate(
    uint8_t *data_out, size_t len_data_out, uint8_t *data_in,
    size_t len_data_in, uint8_t *key, uint32_t len_key,
    SecurityAssociation_t *sa_ptr, // For key index or key references (when key
                                   // not passed in explicitly via key param)
...
static int32_t cryptography_validate_authentication(
  ...
  // Need to copy the data over, since authentication won't change/move the data
  // directly If you don't want data out, don't set a data out length
  ...
  if ((gcry_error & GPG_ERR_CODE_MASK) != GPG_ERR_NO_ERROR) {
    printf(KRED "ERROR: gcry_mac_open error code %d\n" RESET,
           gcry_error & GPG_ERR_CODE_MASK);
    printf(KRED "Failure: %s/%s\n" RESET, gcry_strsource(gcry_error),
           gcry_strerror(gcry_error));
    status = CRYPTO_LIB_ERR_LIBGCRYPT_ERROR;
    return status;
  }
...
static int32_t cryptography_encrypt(uint8_t *data_out, size_t len_data_out,
  ...
  data_out = data_out; // TODO:  Look into tailoring these out, as they're not
                       // used or needed.
  len_data_out = len_data_out;
  padding = padding;
  cam_cookies = cam_cookies;
  ...
  // TODO:  Add PKCS#7 padding to data_in, and increment len_data_in to match
  // necessary block size
  // TODO:  Remember to remove the padding.
  // TODO:  Does this interfere with max frame size?  Does that need to be taken
  // into account? gcry_error = gcry_cipher_encrypt(tmp_hd,
  //                                     data_out,              // ciphertext
  //                                     output len_data_out,                //
  //                                     length of data data_in, // plaintext
  //                                     input len_data_in                 // in
  //                                     data length
  // );
...
int32_t cryptography_verify_ecs_enum_algo(uint8_t *ecs, int32_t *algo,
                                          int32_t *mode) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (ecs != NULL) {
    *algo = cryptography_get_ecs_algo(*ecs);
...
int32_t cryptography_gcry_setup(int32_t mode, int32_t algo,
                                gcry_cipher_hd_t *tmp_hd, uint8_t *key_ptr,
                                uint32_t len_key, uint8_t *iv, uint32_t iv_len,
...
static int32_t cryptography_aead_encrypt(
  ...
  status = cryptography_gcry_setup(mode, algo, &tmp_hd, key_ptr, len_key, iv,
                                   iv_len, &gcry_error);
  if (status != CRYPTO_LIB_SUCCESS) {
    mc_if->mc_log(status);
    return status;
  ...
  if (encrypt_bool == CRYPTO_TRUE) {
    ...
    // necessary block size
    // TODO:  Remember to remove the padding.
    // TODO:  Does this interfere with max frame size?  Does that need to be
    // taken into account?
...
static int32_t cryptography_aead_decrypt(
  ...
  if (algo != GCRY_CIPHER_AES256) {
    printf(
        KRED
        "Warning - only  AES256 supported for AEAD decrypt - exiting!\n" RESET);
    status = CRYPTO_LIB_ERR_UNSUPPORTED_ECS;
    return status;
  }

  gcry_error =
      gcry_cipher_open(&(tmp_hd), GCRY_CIPHER_AES256, mode, GCRY_CIPHER_NONE);
  if ((gcry_error & GPG_ERR_CODE_MASK) != GPG_ERR_NO_ERROR) {
    printf(KRED "ERROR: gcry_cipher_open error code %d\n" RESET,
           gcry_error & GPG_ERR_CODE_MASK);
    printf(KRED "Failure: %s/%s\n", gcry_strsource(gcry_error),
  ...
  {
    // Authenticate only! No input data passed into decryption function, only
    // AAD.
    gcry_error = gcry_cipher_decrypt(tmp_hd, NULL, 0, NULL, 0);
    // If authentication only, don't decrypt the data. Just pass the data PDU
    // through.
    memcpy(data_out, data_in, len_data_in);
  ...
  if (authenticate_bool == CRYPTO_TRUE) {
    /*
    ** *** !!!WARNING!!!
    ** *** This Debug block cannot be enabled during normal use, gettag
...
/**
 * @brief Function: cryptography_get_acs_algo. Maps Cryptolib ACS enums to
 *libgcrypt enums It is possible for supported algos to vary between crypto
 *libraries
 * @param algo_enum
 **/
int32_t cryptography_get_acs_algo(int8_t algo_enum) {
  int32_t algo =
      CRYPTO_LIB_ERR_UNSUPPORTED_ACS; // All valid algos will be positive
  switch (algo_enum) {
...
/**
 * @brief Function: cryptography_get_ecs_algo. Maps Cryptolib ECS enums to
 *libgcrypt enums It is possible for supported algos to vary between crypto
 *libraries
 * @param algo_enum
 **/
int32_t cryptography_get_ecs_algo(int8_t algo_enum) {
  int32_t algo =
      CRYPTO_LIB_ERR_UNSUPPORTED_ECS; // All valid algos will be positive
  switch (algo_enum) {
...
/**
 * @brief Function: cryptography_get_ecs_mode. Maps Cryptolib ECS enums to
 *libgcrypt enums It is possible for supported algos to vary between crypto
 *libraries
 * @param algo_enum
 **/
int32_t cryptography_get_ecs_mode(int8_t algo_enum) {
  int32_t mode =
      CRYPTO_LIB_ERR_UNSUPPORTED_ECS_MODE; // All valid algos will be positive
  switch (algo_enum) {
  case CRYPTO_CIPHER_AES256_GCM:
