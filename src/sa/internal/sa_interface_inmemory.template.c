...
int32_t sa_load_file() {
  ...
  sa_save_file =
      fopen(CRYPTO_SA_SAVE, "rb+"); // Should this be rb instead of wb+
 ...
 * At most an SA can contain 2 unique Keys.  These my not be utilized in another
 * SA
 */
int32_t key_validation(void) {
  ...
  for (i = 0; i < NUM_SA; i++) {
    ...
    for (j = i + 1; j < NUM_SA; j++) {
      ...
      if ((i_ekid == j_ekid) || (i_ekid == j_akid) || (i_akid == j_ekid) ||
          (i_akid == j_akid) || (j_ekid == j_akid)) {
        status = CRYPTO_LIB_ERR_KEY_VALIDATION;
#ifdef DEBUG
        printf(KRED "SA Key Validation FAILURE!\n");
        printf("Key Duplication SA: %d, EKID: %d, AKID: %d\n\tSA: %d, EKID: "
...
static int32_t sa_get_from_spi(uint16_t spi,
  ...
  *security_association = &sa[spi];
  // if (sa[spi].shivf_len > 0 && crypto_config.cryptography_type !=
...
int32_t sa_get_operational_sa_from_gvcid_find_iv(
    uint8_t tfvn, uint16_t scid, uint16_t vcid, uint8_t mapid,
    SecurityAssociation_t **security_association) {
...
void sa_mismatched_tfvn_error(int *i_p, int32_t *status, uint8_t tfvn,
                              uint16_t scid, uint16_t vcid, uint8_t mapid) {
  int i = *i_p;
  if ((sa[i].gvcid_blk.tfvn != tfvn) && (sa[i].gvcid_blk.scid == scid) &&
...
void sa_mismatched_scid(int *i_p, int32_t *status, uint8_t tfvn, uint16_t scid,
                        uint16_t vcid, uint8_t mapid) {
  int i = *i_p;
  if ((sa[i].gvcid_blk.tfvn == tfvn) && (sa[i].gvcid_blk.scid != scid) &&
      (sa[i].gvcid_blk.vcid == vcid) &&
      (sa[i].gvcid_blk.mapid == mapid && sa[i].sa_state == SA_OPERATIONAL)) {
...
void sa_mismatched_vcid(int *i_p, int32_t *status, uint8_t tfvn, uint16_t scid,
                        uint16_t vcid, uint8_t mapid) {
  int i = *i_p;
  if ((sa[i].gvcid_blk.tfvn == tfvn) && (sa[i].gvcid_blk.scid == scid) &&
      (sa[i].gvcid_blk.vcid != vcid) &&
...
void sa_mismatched_mapid(int *i_p, int32_t *status, uint8_t tfvn, uint16_t scid,
                         uint16_t vcid, uint8_t mapid) {
  int i = *i_p;
  if ((sa[i].gvcid_blk.tfvn == tfvn) && (sa[i].gvcid_blk.scid == scid) &&
      (sa[i].gvcid_blk.vcid == vcid) &&
...
void sa_non_operational_sa(int *i_p, int32_t *status, uint8_t tfvn,
                           uint16_t scid, uint16_t vcid, uint8_t mapid) {
  int i = *i_p;
  if ((sa[i].gvcid_blk.tfvn == tfvn) && (sa[i].gvcid_blk.scid == scid) &&
      (sa[i].gvcid_blk.vcid == vcid) &&
...
int32_t sa_get_operational_sa_from_gvcid_generate_error(int32_t *status,
                                                        uint8_t tfvn,
                                                        uint16_t scid,
                                                        uint16_t vcid,
                                                        uint8_t mapid) {
...
#ifdef SA_DEBUG
    printf(KRED
           "Error - Making best attempt at a useful error code:\n\t" RESET);
#endif
    for (i = 0; i < NUM_SA; i++) {
...
sa_get_operational_sa_from_gvcid(uint8_t tfvn, uint16_t scid, uint16_t vcid,
  ...
  status = sa_get_operational_sa_from_gvcid_find_iv(tfvn, scid, vcid, mapid,
                                                    security_association);

  // If not a success, attempt to generate a meaningful error code
  status = sa_get_operational_sa_from_gvcid_generate_error(&status, tfvn, scid,
                                                           vcid, mapid);
...
static int32_t sa_start(TC_t *tc_frame) {
  ...
  // Read ingest
  spi =
      ((uint8_t)sdls_frame.pdu.data[0] << 8) | (uint8_t)sdls_frame.pdu.data[1];

  // Overwrite last PID
  sa[spi].lpid = (sdls_frame.pdu.type << 7) | (sdls_frame.pdu.uf << 6) |
  ...
  if (spi < NUM_SA) {
    if (sa[spi].sa_state == SA_KEYED) {
      ...
      for (x = 0; x <= ((sdls_frame.pdu.pdu_len - 2) / 4);
           x++) { // Read in GVCID
        ...
        // TC
        if (gvcid.vcid !=
            tc_frame->tc_header.vcid) { // Clear all GVCIDs for provided SPI
        ...
        // TM
        if (gvcid.vcid !=
            tm_frame_pri_hdr.vcid) // TODO Check this tm_frame.tm_header.vcid)
...
static int32_t sa_rekey(void) {
  ...
  // Read ingest
  spi = ((uint8_t)sdls_frame.pdu.data[count] << 8) |
        (uint8_t)sdls_frame.pdu.data[count + 1];
  count = count + 2;
  ...
  if (spi < NUM_SA) {
    if (sa[spi].sa_state == SA_UNKEYED) { // Encryption Key
      sa[spi].ekid = ((uint8_t)sdls_frame.pdu.data[count] << 8) |
                     (uint8_t)sdls_frame.pdu.data[count + 1];
      count = count + 2;

      // Authentication Key
      // sa[spi].akid = ((uint8_t)sdls_frame.pdu.data[count] << 8) |
      // (uint8_t)sdls_frame.pdu.data[count+1]; count = count + 2;
...
static int32_t sa_create(void) {
  ...
  sa[spi].abm_len = (uint8_t)((sdls_frame.pdu.data[count] << 8) |
                              (sdls_frame.pdu.data[count + 1]));
  count = count + 2;
  ...
  for (x = 0; x < sa[spi].arsnw_len; x++) {
    sa[spi].arsnw = sa[spi].arsnw | (((uint8_t)sdls_frame.pdu.data[count++])
                                     << (sa[spi].arsnw_len - x));
  }
...
static int32_t sa_setARSNW(void) {
  ...
  if (spi < NUM_SA) {
    ...
    for (x = 0; x < sa[spi].arsnw_len; x++) {
      sa[spi].arsnw =
          (((uint8_t)sdls_frame.pdu.data[x + 3]) << (sa[spi].arsnw_len - x));
    }
  } else {
