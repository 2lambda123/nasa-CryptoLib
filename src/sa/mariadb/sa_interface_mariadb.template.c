...
static int32_t sa_get_operational_sa_from_gvcid(uint8_t, uint16_t, uint16_t,
                                                uint8_t,
...
static const char *SQL_SADB_GET_SA_BY_SPI =
    "SELECT "
    "spi,ekid,akid,sa_state,tfvn,scid,vcid,mapid,lpid,est,ast,shivf_len,shsnf_"
    "len,shplf_len,stmacf_len,ecs_len,HEX(ecs)"
...
static const char *SQL_SADB_GET_SA_BY_GVCID =
    ...
    "len,shplf_len,stmacf_len,ecs_len,HEX(ecs)"
    ",HEX(iv),iv_len,acs_len,HEX(acs),abm_len,HEX(abm),arsn_len,HEX(arsn),arsnw"
    " FROM security_associations WHERE tfvn='%d' AND scid='%d' AND vcid='%d' "
    "AND mapid='%d' AND sa_state='%d'";
...
static int32_t
parse_sa_from_mysql_query(char *query,
                          SecurityAssociation_t **security_association);
static int32_t convert_hexstring_to_byte_array(char *hexstr,
                                               uint8_t *byte_array);
static void convert_byte_array_to_hexstring(void *src_buffer,
...
SaInterface get_sa_interface_mariadb(void) {
  ...
  sa_if_struct.sa_get_operational_sa_from_gvcid =
      sa_get_operational_sa_from_gvcid;
...
static int32_t sa_config(void) { return CRYPTO_LIB_SUCCESS; }
...
static int32_t sa_init(void) {
  ...
  if (sa_mariadb_config != NULL) {
    con = mysql_init(con);
    if (con != NULL) {
      // mysql_options is removed in MariaDB C connector v3, using
      // mysql_optionsv
      //  Lots of small configuration differences between MySQL connector &
      //  MariaDB Connector Only MariaDB Connector is implemented here:
      //  https://wikidev.in/wiki/C/mysql_mysql_h/mysql_options |
      //  https://mariadb.com/kb/en/mysql_optionsv/
      if (sa_mariadb_config->mysql_mtls_key != NULL) {
        mysql_optionsv(con, MYSQL_OPT_SSL_KEY,
                       sa_mariadb_config->mysql_mtls_key);
      }
      if (sa_mariadb_config->mysql_mtls_cert != NULL) {
        mysql_optionsv(con, MYSQL_OPT_SSL_CERT,
      ...
      // error
      fprintf(stderr, "Error: sa_init() MySQL API function mysql_init() "
                      "returned a connection object that is NULL\n");
    }
  }
  return status;
} // end int32_t sa_init()
...
// Security Association Interaction Functions
static int32_t sa_get_from_spi(uint16_t spi,
                               SecurityAssociation_t **security_association) {
...
static int32_t
sa_get_operational_sa_from_gvcid(uint8_t tfvn, uint16_t scid, uint16_t vcid,
                                 uint8_t mapid,
                                 SecurityAssociation_t **security_association) {
  int32_t status = CRYPTO_LIB_SUCCESS;

  char gvcid_query[2048];
  snprintf(gvcid_query, sizeof(gvcid_query), SQL_SADB_GET_SA_BY_GVCID, tfvn,
           scid, vcid, mapid, SA_OPERATIONAL);
...
static int32_t sa_save_sa(SecurityAssociation_t *sa) {
  ...
  if (sa->iv != NULL) {
    snprintf(update_sa_query, sizeof(update_sa_query),
             SQL_SADB_UPDATE_IV_ARC_BY_SPI, iv_h, arsn_h, sa->spi,
             sa->gvcid_blk.tfvn, sa->gvcid_blk.scid, sa->gvcid_blk.vcid,
             sa->gvcid_blk.mapid);

    free(iv_h);
  } else {
    snprintf(update_sa_query, sizeof(update_sa_query),
             SQL_SADB_UPDATE_IV_ARC_BY_SPI_NULL_IV, arsn_h, sa->spi,
             sa->gvcid_blk.tfvn, sa->gvcid_blk.scid, sa->gvcid_blk.vcid,
             sa->gvcid_blk.mapid);
    free(iv_h);
  }
  ...
  // todo - if query fails, need to push failure message to error stack instead
  // of just return code.
...
// Security Association Utility Functions
static int32_t sa_stop(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_start(TC_t *tc_frame) {
  tc_frame = tc_frame;
  return CRYPTO_LIB_SUCCESS;
}
static int32_t sa_expire(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_rekey(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_status(uint8_t *ingest) {
  ingest = ingest;
  return CRYPTO_LIB_SUCCESS;
}
static int32_t sa_create(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_setARSN(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_setARSNW(void) { return CRYPTO_LIB_SUCCESS; }
static int32_t sa_delete(void) { return CRYPTO_LIB_SUCCESS; }
...
parse_sa_from_mysql_query(char *query,
  ...
  char
      *field_names[num_fields]; //[64]; 64 == max length of column name in MySQL

  // TODO -- Need to store mysql query hex string and then malloc sa->iv
  // according to size.
    ...
    for (int i = 0; i < num_fields; i++) {
      ...
      if (strcmp(field_names[i], "ekid") == 0) {
        if (crypto_config.cryptography_type == CRYPTOGRAPHY_TYPE_LIBGCRYPT) {
          sa->ekid = atoi(row[i]);
        } else // Cryptography Type KMC Crypto Service with PKCS12 String Key
               // References
        {
          sa->ekid = 0;
  ...
  // arsnw_len is not necessary for mariadb interface, putty dummy/default value
  // for prints.
  sa->arsnw_len = 1;
...
static int32_t convert_hexstring_to_byte_array(
    char *source_str,
    uint8_t *
        dest_buffer) { // https://stackoverflow.com/questions/3408706/hexadecimal-string-to-byte-array-in-c/56247335#56247335
...
static int32_t finish_with_error(MYSQL **con_loc, int err) {
  fprintf(stderr, "%s\n",
          mysql_error(*con_loc)); // todo - if query fails, need to push failure
                                  // message to error stack
  mysql_close(*con_loc);
  *con_loc = NULL;
  return err;
}
