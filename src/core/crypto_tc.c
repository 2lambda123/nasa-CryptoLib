...
static int32_t crypto_handle_incrementing_nontransmitted_counter(
    uint8_t *dest, uint8_t *src, int src_full_len, int transmitted_len,
    int window);
 ...
 **/
int32_t Crypto_TC_Get_SA_Service_Type(uint8_t *sa_service_type,
  ...
  if ((sa_ptr->est == 0) && (sa_ptr->ast == 0)) {
    ...
    *sa_service_type = SA_AUTHENTICATED_ENCRYPTION;
    ...
    // Leaving for now as it would be cleaner in SA to have an association enum
    // returned I believe
 ...
 **/
int32_t Crypto_TC_Get_Ciper_Mode_TCA(uint8_t sa_service_type,
...
#ifdef TC_DEBUG
    printf(KYEL
           "DEBUG - Received Control/Command frame - nothing to do.\n" RESET);
#endif
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Handle_Enc_Padding(
    uint8_t sa_service_type, uint32_t *pkcs_padding, uint16_t *p_enc_frame_len,
  ...
  if (sa_service_type == SA_ENCRYPTION) {
    ...
    if (sa_ptr->ecs == CRYPTO_CIPHER_AES256_CBC) {
      ...
      *pkcs_padding =
          TC_BLOCK_SIZE -
          *pkcs_padding; // Could potentially need 16 bytes of padding.

      *p_enc_frame_len += *pkcs_padding; // Add the necessary padding to the
                                         // frame_len + new pad length field
...
int32_t Crypto_TC_Frame_Validation(uint16_t *p_enc_frame_len) {
...
#endif
    printf(KRED "Error: New frame would violate maximum tc frame managed "
                "parameter! \n" RESET);
  ...
  if ((*p_enc_frame_len > 1024) && status == CRYPTO_LIB_SUCCESS) {
    printf(KRED "Error: New frame would violate specification max TC frame "
                "size! \n" RESET);
    status = CRYPTO_LIB_ERR_TC_FRAME_SIZE_EXCEEDS_SPEC_LIMIT;
    mc_if->mc_log(status);
  }
 ...
 **/
int32_t Crypto_TC_Accio_Buffer(uint8_t **p_new_enc_frame,
...
int32_t Crypto_TC_ACS_Algo_Check(SecurityAssociation_t *sa_ptr) {
  ...
  if ((sa_ptr->est == 0) && (sa_ptr->ast == 1)) {
    if (sa_ptr->acs_len != 0) {
      if ((sa_ptr->acs == CRYPTO_MAC_CMAC_AES256 ||
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Check_IV_Setup(SecurityAssociation_t *sa_ptr,
                                 uint8_t *p_new_enc_frame, uint16_t *index) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  int i;
...
int32_t Crypto_TC_Do_Encrypt_PLAINTEXT(
    uint8_t sa_service_type, SecurityAssociation_t *sa_ptr, uint16_t *mac_loc,
  ...
  if (sa_service_type != SA_PLAINTEXT) {
    ...
    if (sa_service_type == SA_AUTHENTICATED_ENCRYPTION ||
        sa_service_type == SA_AUTHENTICATION) {
      *mac_loc = TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN +
                 sa_ptr->shivf_len + sa_ptr->shsnf_len + sa_ptr->shplf_len +
                 tf_payload_len;
#ifdef MAC_DEBUG
      ...
      aad_len = TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN +
                sa_ptr->shivf_len + sa_ptr->shsnf_len + sa_ptr->shplf_len;
      if (sa_service_type ==
          SA_AUTHENTICATION) // auth only, we authenticate the payload as part
                             // of the AEAD encrypt call here
      {
    ...
    if (ecs_is_aead_algorithm == CRYPTO_TRUE) {
      ...
      status = cryptography_if->cryptography_aead_encrypt(
          ...
          (uint8_t *)(p_in_frame + TC_FRAME_HEADER_SIZE +
                      segment_hdr_len), // plaintext input
          (size_t)tf_payload_len,       // in data length
          &(ekp->value[0]),             // Key
          Crypto_Get_ECS_Algo_Keylen(
              sa_ptr->ecs),   // Length of key derived from sa_ptr key_ref
    ...
    } else // non aead algorithm
    {
      ...
      if (sa_service_type == SA_ENCRYPTION) {
        ...
        status = cryptography_if->cryptography_encrypt(
            ...
            //(uint8_t*)(p_in_frame + TC_FRAME_HEADER_SIZE + segment_hdr_len),
            //// plaintext input
            (size_t)tf_payload_len, // in data length
            // new_frame_length,
            &(ekp->value[0]), // Key
 ...
 * @return int32: Success/Failure
 **/
void Crypto_TC_Do_Encrypt_NONPLAINTEXT(uint8_t sa_service_type,
                                       SecurityAssociation_t *sa_ptr) {
  if (sa_service_type != SA_PLAINTEXT) {
#ifdef INCREMENT
    if (crypto_config.crypto_increment_nontransmitted_iv ==
    ...
    } else // SA_INCREMENT_NONTRANSMITTED_IV_FALSE
    {
      ...
      if (sa_ptr->shivf_len > 0 && sa_ptr->iv_len != 0) {
        Crypto_increment(sa_ptr->iv + (sa_ptr->iv_len - sa_ptr->shivf_len),
                         sa_ptr->shivf_len);
      }
    }
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Do_Encrypt(uint8_t sa_service_type,
                             SecurityAssociation_t *sa_ptr, uint16_t *mac_loc,
                             uint16_t tf_payload_len, uint8_t segment_hdr_len,
                             uint8_t *p_new_enc_frame, crypto_key_t *ekp,
                             uint8_t **aad, uint8_t ecs_is_aead_algorithm,
                             uint16_t *index_p, const uint8_t *p_in_frame,
                             char *cam_cookies, uint32_t pkcs_padding,
                             uint16_t new_enc_frame_header_field_length,
                             uint16_t *new_fecf) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  uint16_t index = *index_p;
  status = Crypto_TC_Do_Encrypt_PLAINTEXT(
      sa_service_type, sa_ptr, mac_loc, tf_payload_len, segment_hdr_len,
  ...
  */
  ...
  // Only calculate & insert FECF if CryptoLib is configured to do so & gvcid
  // includes FECF.
  if (current_managed_parameters_struct.has_fecf == TC_HAS_FECF) {
#ifdef FECF_DEBUG
    printf(KCYN "Calcing FECF over %d bytes\n" RESET,
           new_enc_frame_header_field_length - 1);
#endif
    if (crypto_config.crypto_create_fecf == CRYPTO_TC_CREATE_FECF_TRUE) {
      *new_fecf = Crypto_Calc_FECF(p_new_enc_frame,
                                   new_enc_frame_header_field_length - 1);
      *(p_new_enc_frame + new_enc_frame_header_field_length - 1) =
          (uint8_t)((*new_fecf & 0xFF00) >> 8);
      *(p_new_enc_frame + new_enc_frame_header_field_length) =
          (uint8_t)(*new_fecf & 0x00FF);
...
int32_t Crypto_TC_Check_Init_Setup(uint16_t in_frame_length) {
  ...
  if ((crypto_config.init_status == UNITIALIZED) || (mc_if == NULL) ||
      (sa_if == NULL)) {
    printf(KRED "ERROR: CryptoLib Configuration Not Set! -- "
                "CRYPTO_LIB_ERR_NO_CONFIG, Will Exit\n" RESET);
    status = CRYPTO_LIB_ERR_NO_CONFIG;
    // Can't mc_log since it's not configured
    return status; // return immediately so a NULL crypto_config is not
                   // dereferenced later
  }

  if (in_frame_length < 5) // Frame length doesn't have enough bytes for TC TF
                           // header -- error out.
  {
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Sanity_Setup(const uint8_t *p_in_frame,
                               const uint16_t in_frame_length) {
 ...
 * @return int32: Success/Failure
 **/
int32_t Crytpo_TC_Validate_TC_Temp_Header(
    const uint16_t in_frame_length, TC_FramePrimaryHeader_t temp_tc_header,
    const uint8_t *p_in_frame, uint8_t *map_id, uint8_t *segmentation_hdr,
    SecurityAssociation_t **sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (in_frame_length <
      temp_tc_header.fl +
          1) // Specified frame length larger than provided frame!
  {
    status =
  ...
  status = Crypto_Get_Managed_Parameters_For_Gvcid(
      temp_tc_header.tfvn, temp_tc_header.scid, temp_tc_header.vcid,
      gvcid_managed_parameters_array, &current_managed_parameters_struct);

  if (status != CRYPTO_LIB_SUCCESS) {
    mc_if->mc_log(status);
    return status;
  } // Unable to get necessary Managed Parameters for TC TF -- return with
    // error.

  if (current_managed_parameters_struct.has_segmentation_hdr ==
      TC_HAS_SEGMENT_HDRS) {
  ...
  status = sa_if->sa_get_operational_sa_from_gvcid(
      temp_tc_header.tfvn, temp_tc_header.scid, temp_tc_header.vcid, *map_id,
      sa_ptr);
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Finalize_Frame_Setup(
    uint8_t sa_service_type, uint32_t *pkcs_padding, uint16_t *p_enc_frame_len,
    uint16_t *new_enc_frame_header_field_length, uint16_t tf_payload_len,
    SecurityAssociation_t **sa_ptr, uint8_t **p_new_enc_frame) {
  uint32_t status = CRYPTO_LIB_SUCCESS;
  status = Crypto_TC_Handle_Enc_Padding(
      sa_service_type, pkcs_padding, p_enc_frame_len,
      new_enc_frame_header_field_length, tf_payload_len, *sa_ptr);
  if (status == CRYPTO_LIB_SUCCESS) {
    status = Crypto_TC_Validate_SA_Service_Type(sa_service_type);
  }
  if (status == CRYPTO_LIB_SUCCESS) {
    // Ensure the frame to be created will not violate managed parameter maximum
    // length
 ...
 * Handles Frame Padding if necessary:  Depends on KMC vs Internal vs Algorithm
 *etc
 * @param pkcs_padding: uint32_t
 * @param sa_ptr: SecurityAssociation_t*
 * @param p_new_enc_frame: uint8_t*
 ...
 * @return int32: Success/Failure
 **/
void Crypto_TC_Handle_Padding(uint32_t pkcs_padding,
                              SecurityAssociation_t *sa_ptr,
                              uint8_t *p_new_enc_frame, uint16_t *index) {
  int i = 0;
  ...
  if (pkcs_padding) {
    ...
    pkcs_padding =
        pkcs_padding & 0x00FFFFFF; // Truncate to be maxiumum of 3 bytes in size
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Set_IV(SecurityAssociation_t *sa_ptr,
                         uint8_t *p_new_enc_frame, uint16_t *index) {
  uint32_t status = CRYPTO_LIB_SUCCESS;
#ifdef SA_DEBUG
  if (sa_ptr->shivf_len > 0) {
 ...
 * Applies Security to incoming frame.  Encryption, Authentication, and
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_ApplySecurity(const uint8_t *p_in_frame,
                                const uint16_t in_frame_length,
                                uint8_t **pp_in_frame,
                                uint16_t *p_enc_frame_len) {
  // Passthrough to maintain original function signature when CAM isn't used.
  return Crypto_TC_ApplySecurity_Cam(p_in_frame, in_frame_length, pp_in_frame,
                                     p_enc_frame_len, NULL);
}
/**
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_ApplySecurity_Cam(const uint8_t *p_in_frame,
                                    const uint16_t in_frame_length,
                                    uint8_t **pp_in_frame,
                                    uint16_t *p_enc_frame_len,
                                    char *cam_cookies) {
  ...
  temp_tc_header.vcid =
      ((uint8_t)p_in_frame[2] & 0xFC) >> 2 & crypto_config.vcid_bitmask;
  ...
  status = Crytpo_TC_Validate_TC_Temp_Header(in_frame_length, temp_tc_header,
                                             p_in_frame, &map_id,
                                             &segmentation_hdr, &sa_ptr);
  if (status != CRYPTO_LIB_SUCCESS) {
    return status;
  }
  ...
  // Determine Algorithm cipher & mode. // TODO - Parse authentication_cipher,
  // and handle AEAD cases properly
  status = Crypto_TC_Get_Ciper_Mode_TCA(sa_service_type, &encryption_cipher,
                                        &ecs_is_aead_algorithm, sa_ptr);
  if (status != CRYPTO_LIB_SUCCESS) {
    mc_if->mc_log(status);
    return status;
  }
  ...
  tf_payload_len =
      temp_tc_header.fl - TC_FRAME_HEADER_SIZE - segment_hdr_len - fecf_len + 1;

  /**
   * A note on plaintext: Take a permissive approach to allow the lengths of
   fields that aren't going to be used.
   * The 355.0-B-2 (July 2022) says the following in $4.2.2.4:
   * 'It is possible to create a ‘clear mode’ SA using one of the defined
   service types by specifying the algorithm as a ‘no-op’ function (no actual
   cryptographic operation to be performed). Such an SA might be used, for
   example, during development testing of other aspects of data link processing
   before cryptographic capabilities are available for integrated testing.In
   this scenario, the Security Header and Trailer field lengths are kept
   constant across all supported configurations. For security reasons, the use
   of such an SA is not recommended in normal operation.'
  */

  // Calculate frame lengths based on SA fields
  *p_enc_frame_len = temp_tc_header.fl + 1 + 2 + sa_ptr->shivf_len +
                     sa_ptr->shsnf_len + sa_ptr->shplf_len + sa_ptr->stmacf_len;
  new_enc_frame_header_field_length = (*p_enc_frame_len) - 1;

  // Finalize frame setup
  status = Crypto_TC_Finalize_Frame_Setup(
      sa_service_type, &pkcs_padding, p_enc_frame_len,
      &new_enc_frame_header_field_length, tf_payload_len, &sa_ptr,
      &p_new_enc_frame);
  if (status != CRYPTO_LIB_SUCCESS) {
  ...
  *(p_new_enc_frame + 2) =
      ((*(p_new_enc_frame + 2) & 0xFC) |
       (((new_enc_frame_header_field_length) & (0x0300)) >> 8));
  ...
  */
  ...
  if (current_managed_parameters_struct.has_segmentation_hdr ==
      TC_HAS_SEGMENT_HDRS) {
    index++; // Add 1 byte to index because segmentation header used for this
             // gvcid.
  }
  ...
  */
  ...
  /* 4.1.1.5.2 The Pad Length field shall contain the count of fill bytes used
  *in the
  ** cryptographic process, consisting of an integral number of octets. - CCSDS
  *3550b1
  */
  ...
  */
  ...
  // tf_payload_len = temp_tc_header.fl - TC_FRAME_HEADER_SIZE - segment_hdr_len
  // - fecf_len + 1;

  memcpy((p_new_enc_frame + index),
         (p_in_frame + TC_FRAME_HEADER_SIZE + segment_hdr_len), tf_payload_len);
  ...
  for (uint32_t i = 0; i < pkcs_padding; i++) {
    ...
    */
    // TODO: Set this depending on crypto cipher used
    *(p_new_enc_frame + index + i) =
        (uint8_t)pkcs_padding; // How much padding is needed?
                               // index++;
  }
  index -= tf_payload_len;
  tf_payload_len += pkcs_padding;
  ...
  */
  status = Crypto_TC_Do_Encrypt(
      sa_service_type, sa_ptr, &mac_loc, tf_payload_len, segment_hdr_len,
      p_new_enc_frame, ekp, &aad, ecs_is_aead_algorithm, &index, p_in_frame,
      cam_cookies, pkcs_padding, new_enc_frame_header_field_length, &new_fecf);
  if (status != CRYPTO_LIB_SUCCESS) {
    mc_if->mc_log(status);
    return status;
  }
  ...
  printf("\n\tThe returned length is: %d\n" RESET,
         new_enc_frame_header_field_length);
#endif
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_ProcessSecurity(uint8_t *ingest, int *len_ingest,
                                  TC_t *tc_sdls_processed_frame) {
  // Pass-through to maintain original function signature when CAM isn't used.
  return Crypto_TC_ProcessSecurity_Cam(ingest, len_ingest,
                                       tc_sdls_processed_frame, NULL);
}
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Parse_Check_FECF(uint8_t *ingest, int *len_ingest,
                                   TC_t *tc_sdls_processed_frame) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (current_managed_parameters_struct.has_fecf == TC_HAS_FECF) {
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Nontransmitted_IV_Increment(SecurityAssociation_t *sa_ptr,
                                              TC_t *tc_sdls_processed_frame) {
  int32_t status = CRYPTO_LIB_SUCCESS;

  if (sa_ptr->shivf_len < sa_ptr->iv_len &&
      crypto_config.ignore_anti_replay == TC_IGNORE_ANTI_REPLAY_FALSE &&
      crypto_config.crypto_increment_nontransmitted_iv ==
          SA_INCREMENT_NONTRANSMITTED_IV_TRUE) {
    status = crypto_handle_incrementing_nontransmitted_counter(
        tc_sdls_processed_frame->tc_sec_header.iv, sa_ptr->iv, sa_ptr->iv_len,
        sa_ptr->shivf_len, sa_ptr->arsnw);
    if (status != CRYPTO_LIB_SUCCESS) {
      mc_if->mc_log(status);
      return status;
    }
  } else // Not checking IV ARSNW or only non-transmitted portion is static;
         // Note, non-transmitted IV in SA must match frame or will fail MAC
         // check.
  {
    // Retrieve non-transmitted portion of IV from SA (if applicable)
    memcpy(tc_sdls_processed_frame->tc_sec_header.iv, sa_ptr->iv,
           sa_ptr->iv_len - sa_ptr->shivf_len);
  }
  return status;
}
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Nontransmitted_SN_Increment(SecurityAssociation_t *sa_ptr,
                                              TC_t *tc_sdls_processed_frame) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_ptr->shsnf_len < sa_ptr->arsn_len &&
      crypto_config.ignore_anti_replay == TC_IGNORE_ANTI_REPLAY_FALSE) {
    status = crypto_handle_incrementing_nontransmitted_counter(
        tc_sdls_processed_frame->tc_sec_header.sn, sa_ptr->arsn,
  ...
  } else // Not checking ARSN in ARSNW
  {
    ...
    memcpy(tc_sdls_processed_frame->tc_sec_header.sn, sa_ptr->arsn,
           sa_ptr->arsn_len - sa_ptr->shsnf_len);
  }
  return status;
}
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Check_ACS_Keylen(crypto_key_t *akp,
                                   SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if ((int32_t)akp->key_len != Crypto_Get_ACS_Algo_Keylen(sa_ptr->acs)) {
    status = CRYPTO_LIB_ERR_KEY_LENGTH_ERROR;
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_Check_ECS_Keylen(crypto_key_t *ekp,
                                   SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if ((int32_t)ekp->key_len != Crypto_Get_ECS_Algo_Keylen(sa_ptr->ecs)) {
    status = CRYPTO_LIB_ERR_KEY_LENGTH_ERROR;
 ...
 **/
int32_t Crypto_TC_Do_Decrypt(uint8_t sa_service_type,
                             uint8_t ecs_is_aead_algorithm, crypto_key_t *ekp,
                             SecurityAssociation_t *sa_ptr, uint8_t *aad,
                             TC_t *tc_sdls_processed_frame, uint8_t *ingest,
                             uint16_t tc_enc_payload_start_index,
                             uint16_t aad_len, char *cam_cookies,
  ...
  if (sa_service_type != SA_PLAINTEXT && ecs_is_aead_algorithm == CRYPTO_TRUE) {
    ...
    status = cryptography_if->cryptography_aead_decrypt(
        ...
        cam_cookies    //
             ...
             ecs_is_aead_algorithm == CRYPTO_FALSE) // Non aead algorithm
  {
    // TODO - implement non-AEAD algorithm logic
    if (sa_service_type == SA_AUTHENTICATION ||
        sa_service_type == SA_AUTHENTICATED_ENCRYPTION) {
    ...
    if (sa_service_type == SA_ENCRYPTION ||
      ...
      if (sa_ptr->shplf_len != 0) {
        int padding_location = TC_FRAME_HEADER_SIZE + segment_hdr_len +
                               SPI_LEN + sa_ptr->shivf_len + sa_ptr->shsnf_len;
    ...
    memcpy(tc_sdls_processed_frame->tc_pdu,
           &(ingest[tc_enc_payload_start_index]),
...
int32_t Crypto_TC_Process_Sanity_Check(int *len_ingest) {
  ...
  if ((*len_ingest < 5) &&
      (status == CRYPTO_LIB_SUCCESS)) // Frame length doesn't even have enough
                                      // bytes for header -- error out.
  {
    status = CRYPTO_LIB_ERR_INPUT_FRAME_TOO_SHORT_FOR_TC_STANDARD;
    mc_if->mc_log(status);
  }
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TC_Prep_AAD(TC_t *tc_sdls_processed_frame, uint8_t fecf_len,
                           uint8_t sa_service_type,
                           uint8_t ecs_is_aead_algorithm, uint16_t *aad_len,
                           SecurityAssociation_t *sa_ptr,
                           uint8_t segment_hdr_len, uint8_t *ingest,
                           uint8_t **aad) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  ...
  if ((sa_service_type == SA_AUTHENTICATION) ||
      (sa_service_type == SA_AUTHENTICATED_ENCRYPTION)) {
    uint16_t tc_mac_start_index = tc_sdls_processed_frame->tc_header.fl + 1 -
                                  fecf_len - sa_ptr->stmacf_len;
    ...
    Crypto_hexprint(tc_sdls_processed_frame->tc_sec_trailer.mac,
                    sa_ptr->stmacf_len);
#endif
    ...
    if ((sa_service_type == SA_AUTHENTICATED_ENCRYPTION) &&
      ...
      aad_len_temp = TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN +
                     sa_ptr->shivf_len + sa_ptr->shsnf_len + sa_ptr->shplf_len;
    }
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TC_Get_Keys(crypto_key_t **ekp, crypto_key_t **akp,
                           SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  *ekp = key_if->get_key(sa_ptr->ekid);
  *akp = key_if->get_key(sa_ptr->akid);
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TC_Check_IV_ARSN(SecurityAssociation_t *sa_ptr,
                                TC_t *tc_sdls_processed_frame) {
  int32_t status = CRYPTO_LIB_SUCCESS;

  if (crypto_config.ignore_anti_replay == TC_IGNORE_ANTI_REPLAY_FALSE &&
      status == CRYPTO_LIB_SUCCESS) {
    status = Crypto_Check_Anti_Replay(
        sa_ptr, tc_sdls_processed_frame->tc_sec_header.sn,
    ...
    {
      // Only save the SA (IV/ARSN) if checking the anti-replay counter;
 ...
 * @return int32_t: Success/Failure
 **/
uint32_t Crypto_TC_Sanity_Validations(TC_t *tc_sdls_processed_frame,
                                      SecurityAssociation_t **sa_ptr) {
  uint32_t status = CRYPTO_LIB_SUCCESS;

  status = sa_if->sa_get_from_spi(tc_sdls_processed_frame->tc_sec_header.spi,
                                  sa_ptr);
 ...
 * @param sa_ptr:SecurityAssociation_t*
 **/
void Crypto_TC_Get_Ciper_Mode_TCP(uint8_t sa_service_type,
                                  uint32_t *encryption_cipher,
                                  uint8_t *ecs_is_aead_algorithm,
                                  SecurityAssociation_t *sa_ptr) {
  if (sa_service_type != SA_PLAINTEXT) {
 ...
 * @param byte_idx: int*
 **/
void Crypto_TC_Set_Segment_Header(TC_t *tc_sdls_processed_frame,
                                  uint8_t *ingest, int *byte_idx) {
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TC_ProcessSecurity_Cam(uint8_t *ingest, int *len_ingest,
                                      TC_t *tc_sdls_processed_frame,
                                      char *cam_cookies)
// Loads the ingest frame into the global tc_frame while performing decryption
{
  ...
  // Primary Header
  tc_sdls_processed_frame->tc_header.tfvn =
      ((uint8_t)ingest[byte_idx] & 0xC0) >> 6;
  tc_sdls_processed_frame->tc_header.bypass =
      ((uint8_t)ingest[byte_idx] & 0x20) >> 5;
  tc_sdls_processed_frame->tc_header.cc =
      ((uint8_t)ingest[byte_idx] & 0x10) >> 4;
  tc_sdls_processed_frame->tc_header.spare =
      ((uint8_t)ingest[byte_idx] & 0x0C) >> 2;
  tc_sdls_processed_frame->tc_header.scid = ((uint8_t)ingest[byte_idx] & 0x03)
                                            << 8;
  byte_idx++;
  tc_sdls_processed_frame->tc_header.scid =
      tc_sdls_processed_frame->tc_header.scid | (uint8_t)ingest[byte_idx];
  byte_idx++;
  tc_sdls_processed_frame->tc_header.vcid =
      (((uint8_t)ingest[byte_idx] & 0xFC) >> 2) & crypto_config.vcid_bitmask;
  tc_sdls_processed_frame->tc_header.fl = ((uint8_t)ingest[byte_idx] & 0x03)
                                          << 8;
  byte_idx++;
  tc_sdls_processed_frame->tc_header.fl =
      tc_sdls_processed_frame->tc_header.fl | (uint8_t)ingest[byte_idx];
  byte_idx++;
  tc_sdls_processed_frame->tc_header.fsn = (uint8_t)ingest[byte_idx];
  byte_idx++;

  if (*len_ingest < tc_sdls_processed_frame->tc_header.fl +
                        1) // Specified frame length larger than provided frame!
  {
    status =
        CRYPTO_LIB_ERR_INPUT_FRAME_LENGTH_SHORTER_THAN_FRAME_HEADERS_LENGTH;
  ...
  status = Crypto_Get_Managed_Parameters_For_Gvcid(
      tc_sdls_processed_frame->tc_header.tfvn,
      tc_sdls_processed_frame->tc_header.scid,
      tc_sdls_processed_frame->tc_header.vcid, gvcid_managed_parameters_array,
  ...
  tc_sdls_processed_frame->tc_sec_header.spi =
      ((uint8_t)ingest[byte_idx] << 8) | (uint8_t)ingest[byte_idx + 1];
  byte_idx += 2;
  ...
  // Allocate the necessary byte arrays within the security header + trailer
  // given the SA
  ...
  // tc_sdls_processed_frame->tc_sec_trailer.mac = calloc(1,
  // sa_ptr->stmacf_len);
  // Set tc_sec_header + trailer fields for actual lengths from the SA
  // (downstream apps won't know this length otherwise since they don't access
  // the SADB!).
  ...
  // Determine Algorithm cipher & mode. // TODO - Parse authentication_cipher,
  // and handle AEAD cases properly
  Crypto_TC_Get_Ciper_Mode_TCP(sa_service_type, &encryption_cipher,
                               &ecs_is_aead_algorithm, sa_ptr);
  ...
  // Parse transmitted portion of IV from received frame (Will be Whole IV if
  // iv_len==shivf_len)
  memcpy((tc_sdls_processed_frame->tc_sec_header.iv +
          (sa_ptr->iv_len - sa_ptr->shivf_len)),
         &(ingest[TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN]),
         sa_ptr->shivf_len);

  // Handle non-transmitted IV increment case (transmitted-portion roll-over)
  status =
      Crypto_TC_Nontransmitted_IV_Increment(sa_ptr, tc_sdls_processed_frame);
  if (status != CRYPTO_LIB_SUCCESS) {
    return status;
  }
  ...
  memcpy((tc_sdls_processed_frame->tc_sec_header.sn +
          (sa_ptr->arsn_len - sa_ptr->shsnf_len)),
         &(ingest[TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN +
                  sa_ptr->shivf_len]),
         sa_ptr->shsnf_len);
  ...
  status =
      Crypto_TC_Nontransmitted_SN_Increment(sa_ptr, tc_sdls_processed_frame);
  if (status != CRYPTO_LIB_SUCCESS) {
    return status;
  }
  ...
  // Parse pad length
  // tc_sdls_processed_frame->tc_sec_header.pad = malloc((sa_ptr->shplf_len *
  // sizeof(uint8_t)));
  memcpy((tc_sdls_processed_frame->tc_sec_header.pad),
         &(ingest[TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN +
                  sa_ptr->shivf_len + sa_ptr->shsnf_len]),
         sa_ptr->shplf_len);

  // Parse MAC, prepare AAD
  status = Crypto_TC_Prep_AAD(tc_sdls_processed_frame, fecf_len,
                              sa_service_type, ecs_is_aead_algorithm, &aad_len,
  ...
  uint16_t tc_enc_payload_start_index = TC_FRAME_HEADER_SIZE + segment_hdr_len +
                                        SPI_LEN + sa_ptr->shivf_len +
                                        sa_ptr->shsnf_len + sa_ptr->shplf_len;

  // Todo -- if encrypt only, ignore stmacf_len entirely to avoid erroring on SA
  // misconfiguration... Or just throw a warning/error indicating SA
  // misconfiguration?
  tc_sdls_processed_frame->tc_pdu_len = tc_sdls_processed_frame->tc_header.fl +
                                        1 - tc_enc_payload_start_index -
                                        sa_ptr->stmacf_len - fecf_len;

  if (tc_sdls_processed_frame->tc_pdu_len >
      tc_sdls_processed_frame->tc_header
          .fl) // invalid header parsed, sizes overflowed & make no sense!
  {
  ...
  status = Crypto_TC_Do_Decrypt(sa_service_type, ecs_is_aead_algorithm, ekp,
                                sa_ptr, aad, tc_sdls_processed_frame, ingest,
                                tc_enc_payload_start_index, aad_len,
                                cam_cookies, akp, segment_hdr_len);
  if (status != CRYPTO_LIB_SUCCESS) {
    Crypto_TC_Safe_Free_Ptr(aad);
    mc_if->mc_log(status);
    return status; // Cryptography IF call failed, return.
  ...
  if (status == CRYPTO_LIB_SUCCESS &&
      crypto_config.process_sdls_pdus == TC_PROCESS_SDLS_PDUS_TRUE) {
    status =
        Crypto_Process_Extended_Procedure_Pdu(tc_sdls_processed_frame, ingest);
  }
 ...
 **/
/*
int32_t Crypto_Get_tcPayloadLength(TC_t* tc_frame, SecurityAssociation_t*
sa_ptr)
{
    int tf_hdr = 5;
    int seg_hdr =
0;if(current_managed_parameters_struct.has_segmentation_hdr==TC_HAS_SEGMENT_HDRS){seg_hdr=1;}
    int fecf =
0;if(current_managed_parameters_struct.has_fecf==TC_HAS_FECF){fecf=FECF_SIZE;}
    int spi = 2;
    ...
    int mac_size = sa_ptr->stmacf_len;
        ...
        printf("\tmac\t%d\n",mac_size);
        printf("\tfecf \t%d\n",fecf);
        printf("\tTOTAL LENGTH: %d\n", (tc_frame->tc_header.fl - (tf_hdr +
seg_hdr + spi + iv_size ) - (mac_size + fecf))); #endif

    return (tc_frame->tc_header.fl + 1 - (tf_hdr + seg_hdr + spi + iv_size ) -
(mac_size + fecf) );
}
*/
 ...
 * Callocs and returns pointer to buffer where AAD is created & bitwise-anded
 *with bitmask! Note: Function caller is responsible for freeing the returned
 *buffer!
 * @param buffer: uint8_t*
 * @param len_aad: uint16_t
 * @param abm_buffer: uint8_t*
 **/
uint8_t *Crypto_Prepare_TC_AAD(uint8_t *buffer, uint16_t len_aad,
                               uint8_t *abm_buffer) {
...
static int32_t crypto_tc_validate_sa(SecurityAssociation_t *sa) {
  ...
  if (crypto_config.iv_type == IV_CRYPTO_MODULE &&
      crypto_config.cryptography_type == CRYPTOGRAPHY_TYPE_LIBGCRYPT) {
    return CRYPTO_LIB_ERR_NULL_IV;
  }
  ...
  for (int i = 0; i < window; i++) {
    ...
    if (counter_matches == CRYPTO_TRUE) {
      break;
               ...
               window -
                   1) // Only reset flag if there are more  windows to check.
    {
      counter_matches =
          CRYPTO_TRUE; // reset the flag, and continue the for loop for the next
      continue;
    }
  }

  if (counter_matches == CRYPTO_TRUE) {
    // Retrieve non-transmitted portion of incremented counter that matches (and
    // may have rolled over/incremented)
