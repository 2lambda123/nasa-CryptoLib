...
char *crypto_enum_errlist_core[] = {
    ...
    (char *)"CRYPTO_LIB_ERR_INPUT_FRAME_LENGTH_SHORTER_THAN_FRAME_HEADERS_"
            "LENGTH",
...
/*
** @brief: Helper Function. Get specific error code, given code, allowable max,
*and valid string expansion
** @param: int32_t, int32_t, char*
* @return: char*
*/
char *Crypto_Get_Crypto_Error_Code_String(int32_t crypto_error_code,
                                          int32_t crypto_error_code_max,
...
* @return: char*
*/
char *Crypto_Get_Error_Code_String(int32_t crypto_error_code,
                                   int32_t crypto_error_code_max,
                                   char *valid_output_string) {
  if (crypto_error_code > crypto_error_code_max) {
    return CRYPTO_UNDEFINED_ERROR;
...
/*
** @brief: For a given crypto error code, return the associated error code enum
*string
** @param: int32_t
* @return: char*
*/
char *Crypto_Get_Error_Code_Enum_String(int32_t crypto_error_code) {
  ...
  if (crypto_error_code >= 600) // CAM Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 610,
        crypto_enum_errlist_crypto_cam[crypto_error_code % 600]);
  } else if (crypto_error_code >= 500) // KMC Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 515,
        crypto_enum_errlist_crypto_kmc[crypto_error_code % 500]);
  } else if (crypto_error_code >= 400) // Crypto Interface Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 402,
        crypto_enum_errlist_crypto_if[crypto_error_code % 400]);
  } else if (crypto_error_code >= 300) // SADB MariadDB Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 303,
        crypto_enum_errlist_sa_mariadb[crypto_error_code % 300]);
  } else if (crypto_error_code >= 200) // SADB Interface Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 201,
        crypto_enum_errlist_sa_if[crypto_error_code % 200]);
  } else if (crypto_error_code >= 100) // Configuration Error Codes
  {
    return_string = Crypto_Get_Error_Code_String(
        crypto_error_code, 103,
        crypto_enum_errlist_config[crypto_error_code % 100]);
  } else if (crypto_error_code <= 0) // Cryptolib Core Error Codes
  {
    return_string = Crypto_Get_Crypto_Error_Code_String(
        crypto_error_code, -51,
        crypto_enum_errlist_core[(crypto_error_code * (-1))]);
  }
