...
int32_t Crypto_SA_readARSN(uint8_t *ingest) {
  ...
  if (sa_ptr->shivf_len > 0) { // Set IV - authenticated encryption
    for (x = 0; x < sa_ptr->shivf_len - 1; x++) {
      ingest[count++] = *(sa_ptr->iv + x);
    }

    // TODO: Do we need this?
    if (*(sa_ptr->iv + sa_ptr->shivf_len - 1) >
        0) { // Adjust to report last received, not expected
 ...
 **/
int32_t Crypto_MC_resetalarm(void) { // Reset all alarm flags
  report.af = 0;
  report.bsnf = 0;
  report.bmacf = 0;
  report.bsaf = 0;
