 ...
 *
 * The AOS ApplySecurity Payload shall consist of the portion of the AOS
 *Transfer Frame (see reference [1]) from the first octet of the Transfer Frame
 *Primary Header to the last octet of the Transfer Frame Data Field. NOTES 1 The
 *AOS Transfer Frame is the fixed-length protocol data unit of the AOS Space
 *Data Link Protocol. The length of any Transfer Frame transferred on a physical
 *channel is constant, and is established by management. 2 The portion of the
 *AOS Transfer Frame contained in the AOS ApplySecurity Payload parameter
 *includes the Security Header field. When the ApplySecurity Function is called,
 *the Security Header field is empty; i.e., the caller has not set any values in
 *the Security Header
 **/
int32_t Crypto_AOS_ApplySecurity(uint8_t *pTfBuffer) {
  ...
  scid = (((uint16_t)pTfBuffer[0] & 0x3F) << 2) |
         (((uint16_t)pTfBuffer[1] & 0xC0) >> 6);
  ...
  if (current_managed_parameters_struct.has_ocf == AOS_HAS_OCF) {
    ...
    printf(KYEL "OCF Location is: %d" RESET,
  ...
  if (sa_service_type != SA_PLAINTEXT) {
    ...
    if (sa_service_type == SA_AUTHENTICATED_ENCRYPTION ||
      ...
      status =
          Crypto_Prepare_AOS_AAD(&pTfBuffer[0], aad_len, sa_ptr->abm, &aad[0]);
    }
  }
...
- 1) + 1;
    ...
    // Header
    tempAOS[count++] = (uint8_t)((tm_frame.tm_header.tfvn << 6) |
((tm_frame.tm_header.scid & 0x3F0) >> 4)); printf("LINE: %d\n",__LINE__);
    tempAOS[count++] = (uint8_t)(((tm_frame.tm_header.scid & 0x00F) << 4) |
(tm_frame.tm_header.vcid << 1) | (tm_frame.tm_header.ocff)); tempAOS[count++] =
(uint8_t)(tm_frame.tm_header.mcfc); tempAOS[count++] =
(uint8_t)(tm_frame.tm_header.vcfc); tempAOS[count++] =
        (uint8_t)((tm_frame.tm_header.tfsh << 7) | (tm_frame.tm_header.sf << 6)
| (tm_frame.tm_header.pof << 5) | (tm_frame.tm_header.slid << 3) |
((tm_frame.tm_header.fhp & 0x700) >> 8)); tempAOS[count++] =
(uint8_t)(tm_frame.tm_header.fhp & 0x0FF);
    //	tempAOS[count++] = (uint8_t) ((tm_frame.tm_header.tfshvn << 6) |
aos_frame.tm_header.tfshlen);
    // Security Header
    printf("LINE: %d\n",__LINE__);
    ...
    // TODO: Troubleshoot
    // Padding Length
    // pad_len = Crypto_Get_tmLength(*len_ingest) - AOS_MIN_SIZE + IV_SIZE +
AOS_PAD_SIZE -*len_ingest; printf("LINE: %d\n",__LINE__);
...
ESA testing printf("LINE: %d\n",__LINE__); tempAOS[count++] = 0x00; // pad_len
>> 8; tempAOS[count++] = 0x1A; // pad_len pdu_loc = count; pdu_len =*len_ingest
+ pad_len;
    }
    printf("LINE: %d\n",__LINE__);
    // Payload Data Unit
...
AAD sa_ptr->ecs, // encryption cipher sa_ptr->acs,  // authentication cipher
                                                           NULL // cam_cookies
(not supported in AOS functions yet)
                                                           );
        ...
        // Update FECF
        aos_frame.tm_sec_trailer.fecf = Crypto_Calc_FECF((uint8_t*)ingest,
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_AOS_ProcessSecurity(uint8_t *p_ingest, uint16_t len_ingest,
  ...
  aos_frame_pri_hdr.scid = (((uint16_t)p_ingest[0] & 0x3F) << 4) |
  ...
  printf(KGRN "tvfn: %d\t scid: %d\t vcid: %d\n" RESET, aos_frame_pri_hdr.tfvn,
         aos_frame_pri_hdr.scid, aos_frame_pri_hdr.vcid);
#endif
  ...
  if (status != CRYPTO_LIB_SUCCESS) {
    ...
    mc_if->mc_log(status);
    return status;
  } // Unable to get necessary Managed Parameters for AOS TF -- return with
    // error.
...
#ifdef AOS_DEBUG
    printf(KRED "AOS_Process Error...tfvn: %d scid: 0x%04X vcid: 0x%02X "
                "fecf_enum: %d\n" RESET,
  ...
  if (current_managed_parameters_struct.aos_has_iz == AOS_HAS_IZ) {
    memcpy(p_new_dec_frame + 6, &p_ingest[6],
...
int32_t Crypto_Get_aosLength(int len) {
  ...
  len = AOS_FILL_SIZE;
#else
  len = AOS_FRAME_PRIMARYHEADER_SIZE + AOS_FRAME_SECHEADER_SIZE + len +
        AOS_FRAME_SECTRAILER_SIZE + AOS_FRAME_CLCW_SIZE;
#endif
...
aos_offset]); printf("%02X", (uint8_t)ingest[x - aos_offset]);
...
void Crypto_AOS_updateOCF(void)
{
    ...
    { // CLCW
        ...
        aos_frame.tm_sec_trailer.ocf[0] = (clcw.cwt << 7) | (clcw.cvn << 5) |
(clcw.sf << 2) | (clcw.cie); aos_frame.tm_sec_trailer.ocf[1] = (clcw.vci << 2) |
(clcw.spare0); aos_frame.tm_sec_trailer.ocf[2] = (clcw.nrfa << 7) | (clcw.nbl <<
6) | (clcw.lo << 5) | (clcw.wait << 4) | (clcw.rt << 3) | (clcw.fbc << 1) |
(clcw.spare1); aos_frame.tm_sec_trailer.ocf[3] = (clcw.rv);
        // Alternate OCF
        ocf = 1;
#ifdef OCF_DEBUG
    ...
    { // FSR
        aos_frame.tm_sec_trailer.ocf[0] = (report.cwt << 7) | (report.vnum << 4)
| (report.af << 3) | (report.bsnf << 2) | (report.bmacf << 1) | (report.ispif);
