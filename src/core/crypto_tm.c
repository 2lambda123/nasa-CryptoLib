/** Copyright (C) 2009 - 2022 National Aeronautics and Space Administration.
 ...
 resulting from, or in any way connected with the software or its documentation,
 whether or not based upon warranty, contract, tort or otherwise, and whether or
 not loss was sustained from, or arose out of the results of, or use of, the
 software, documentation or services provided hereunder.
...
int32_t Crypto_TM_Sanity_Check(uint8_t *pTfBuffer) {
  ...
  if ((status == CRYPTO_LIB_SUCCESS) &&
      ((crypto_config.init_status == UNITIALIZED) || (mc_if == NULL) ||
       (sa_if == NULL))) {
 ...
 **/
int32_t Crypto_TM_Determine_SA_Service_Type(uint8_t *sa_service_type,
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_IV_Sanity_Check(uint8_t *sa_service_type,
                                  SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
#ifdef SA_DEBUG
...
#endif
  if (*sa_service_type != SA_PLAINTEXT && sa_ptr->ecs_len == 0 &&
      sa_ptr->acs_len == 0) {
    status = CRYPTO_LIB_ERR_NULL_CIPHERS;
#ifdef TM_DEBUG
    printf(KRED
           "CRYPTO_LIB_ERR_NULL_CIPHERS, Invalid cipher lengths, %d\n" RESET,
 ...
 **/
void Crypto_TM_PKCS_Padding(uint32_t *pkcs_padding,
                            SecurityAssociation_t *sa_ptr, uint8_t *pTfBuffer,
                            uint16_t *idx_p) {
  uint16_t idx = *idx_p;
  if (*pkcs_padding) {
    uint8_t hex_padding[3] = {0}; // TODO: Create #Define for the 3
    *pkcs_padding = *pkcs_padding &
                    0x00FFFFFF; // Truncate to be maxiumum of 3 bytes in size
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Get_Keys(crypto_key_t **ekp, crypto_key_t **akp,
                           SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
 ...
 * Preps Encryption for Non-plain-text Authentication and Authenticated
 *Encryption
 * @param sa_service_type: uint8_t
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Do_Encrypt_NONPLAINTEXT(uint8_t sa_service_type,
                                          uint16_t *aad_len, int *mac_loc,
                                          uint16_t *idx_p, uint16_t pdu_len,
                                          uint8_t *pTfBuffer, uint8_t *aad,
                                          SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  int16_t idx = *idx_p;
 ...
 * Preps Encryption for Non-plain-text Encryption and Authenticated Encryption
 *for AEAD Algorithms
 * @param sa_service_type: uint8_t
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Do_Encrypt_NONPLAINTEXT_AEAD_Logic(
    uint8_t sa_service_type, uint8_t ecs_is_aead_algorithm, uint8_t *pTfBuffer,
    uint16_t pdu_len, uint16_t data_loc, crypto_key_t *ekp, crypto_key_t *akp,
    uint32_t pkcs_padding, int *mac_loc, uint16_t *aad_len, uint8_t *aad,
    SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;

  if (sa_service_type != SA_PLAINTEXT && ecs_is_aead_algorithm == CRYPTO_TRUE) {
    if (sa_service_type == SA_ENCRYPTION) {
      status = cryptography_if->cryptography_encrypt( // Stub out data in/out as
                                                      // this is done in place
                                                      // and want to save cycles
          (uint8_t *)(&pTfBuffer[data_loc]), // ciphertext output
          (size_t)pdu_len,                   // length of data
          (uint8_t *)(&pTfBuffer[data_loc]), // plaintext input
          (size_t)
              pdu_len, // in data length - from start of frame to end of data
          &(ekp->value[0]), // Key
           ...
           ecs_is_aead_algorithm == CRYPTO_FALSE) // Non aead algorithm
  {
    ...
    if (sa_service_type == SA_AUTHENTICATION) {
      status =
          cryptography_if->cryptography_authenticate( // Stub out data in/out as
                                                      // this is done in place
                                                      // and want to save cycles
              ...
              *aad_len,             // Length of AAD
              ...
              sa_ptr->acs,          // authentication cipher
              NULL);
    } else if (sa_service_type == SA_ENCRYPTION ||
               sa_service_type == SA_AUTHENTICATED_ENCRYPTION) {
      if (sa_service_type == SA_ENCRYPTION) {
        status =
            cryptography_if
                ->cryptography_encrypt( // Stub out data in/out as this is done
                                        // in place and want to save cycles
                    (uint8_t *)(&pTfBuffer[data_loc]), // ciphertext output
                    (size_t)pdu_len,                   // length of data
                    (uint8_t *)(&pTfBuffer[data_loc]), // plaintext input
                    (size_t)pdu_len,  // in data length - from start of frame to
                                      // end of data
                    &(ekp->value[0]), // Key
                    Crypto_Get_ECS_Algo_Keylen(sa_ptr->ecs),
                    sa_ptr,         // SA (for key reference)
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Do_Encrypt_Handle_Increment(uint8_t sa_service_type,
                                              SecurityAssociation_t *sa_ptr) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_service_type != SA_PLAINTEXT) {
#ifdef INCREMENT
    ...
    if (sa_ptr->iv_len > 0) {
      ...
      for (int i = sa_ptr->iv_len - sa_ptr->shivf_len; i < sa_ptr->iv_len;
           i++) {
        printf("%02x", *(sa_ptr->iv + i));
      }
      printf("\n" RESET);
    ...
    for (int i = sa_ptr->arsn_len - sa_ptr->shsnf_len; i < sa_ptr->arsn_len;
         i++) {
      printf("%02x", *(sa_ptr->arsn + i));
    }
    printf("\n" RESET);
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Do_Encrypt(uint8_t sa_service_type,
                             SecurityAssociation_t *sa_ptr, uint16_t *aad_len,
                             int *mac_loc, uint16_t *idx_p, uint16_t pdu_len,
                             uint8_t *pTfBuffer, uint8_t *aad,
                             uint8_t ecs_is_aead_algorithm, uint16_t data_loc,
                             crypto_key_t *ekp, crypto_key_t *akp,
  ...
  int32_t status = CRYPTO_LIB_SUCCESS;
  status =
      Crypto_TM_Do_Encrypt_NONPLAINTEXT(sa_service_type, aad_len, mac_loc,
                                        idx_p, pdu_len, pTfBuffer, aad, sa_ptr);

  // AEAD Algorithm Logic
  if (status == CRYPTO_LIB_SUCCESS) {
    status = Crypto_TM_Do_Encrypt_NONPLAINTEXT_AEAD_Logic(
        sa_service_type, ecs_is_aead_algorithm, pTfBuffer, pdu_len, data_loc,
        ekp, akp, pkcs_padding, mac_loc, aad_len, aad, sa_ptr);
  }
  ...
  if (status == CRYPTO_LIB_SUCCESS) {
    ...
    if (current_managed_parameters_struct.has_fecf == TM_HAS_FECF) {
      ...
      printf(KCYN "Calcing FECF over %d bytes\n" RESET,
             current_managed_parameters_struct.max_frame_size - 2);
#endif
      if (crypto_config.crypto_create_fecf == CRYPTO_TM_CREATE_FECF_TRUE) {
        *new_fecf = Crypto_Calc_FECF(
            (uint8_t *)pTfBuffer,
            current_managed_parameters_struct.max_frame_size - 2);
        pTfBuffer[current_managed_parameters_struct.max_frame_size - 2] =
            (uint8_t)((*new_fecf & 0xFF00) >> 8);
        pTfBuffer[current_managed_parameters_struct.max_frame_size - 1] =
 ...
 * @param sa_ptr: SecurityAssociation_t*
 **/
void Crypto_TM_ApplySecurity_Debug_Print(uint16_t idx, uint16_t pdu_len,
                                         SecurityAssociation_t *sa_ptr) {
 ...
 * @return int32: Success/Failure
 *
 * The TM ApplySecurity Payload shall consist of the portion of the TM Transfer
 *Frame (see reference [1]) from the first octet of the Transfer Frame Primary
 *Header to the last octet of the Transfer Frame Data Field. NOTES 1 The TM
 *Transfer Frame is the fixed-length protocol data unit of the TM Space Data
 * Link Protocol. The length of any Transfer Frame transferred on a physical
 *channel is constant, and is established by management. 2 The portion of the TM
 *Transfer Frame contained in the TM ApplySecurity Payload parameter includes
 *the Security Header field. When the ApplySecurity Function is called, the
 *Security Header field is empty; i.e., the caller has not set any values in the
 * Security Header
 **/
int32_t Crypto_TM_ApplySecurity(uint8_t *pTfBuffer) {
  ...
  scid = (((uint16_t)pTfBuffer[0] & 0x3F) << 4) |
         (((uint16_t)pTfBuffer[1] & 0xF0) >> 4);
  ...
  status =
      sa_if->sa_get_operational_sa_from_gvcid(tfvn, scid, vcid, 0, &sa_ptr);
  ...
  status = Crypto_Get_Managed_Parameters_For_Gvcid(
      tfvn, scid, vcid, gvcid_managed_parameters_array,
  ...
  for (int16_t i = 0; i < current_managed_parameters_struct.max_frame_size;
       i++) {
    printf("%02X", pTfBuffer[i]);
  }
  printf("\n");
  ...
  // Note: Secondary headers are static only for a mission phase, not guaranteed
  // static over the life of a mission Per CCSDS 132.0-B.3 Section 4.1.2.7.2.3
   ...
   *in the cryptographic process, consisting of an integral number of octets. -
   *CCSDS 3550b1
   **/
  ...
  //  {
  //      /** 4.1.1.5.2 The Pad Length field shall contain the count of fill
  //      bytes used in the
  //       * cryptographic process, consisting of an integral number of octets.
  //       - CCSDS 3550b1
  //       **/
  //      // TODO: Set this depending on crypto cipher used
  //     * (p_new_enc_frame + index + i) = (uint8_t)pkcs_padding; // How much
  //     padding is needed?
  //      // index++;
  //  }
  ...
  pdu_len = current_managed_parameters_struct.max_frame_size - idx -
            sa_ptr->stmacf_len;
  // Check other managed parameter flags, subtract their lengths from data field
  // if present
  ...
  status = Crypto_TM_Do_Encrypt(
      sa_service_type, sa_ptr, &aad_len, &mac_loc, &idx, pdu_len, pTfBuffer,
      aad, ecs_is_aead_algorithm, data_loc, ekp, akp, pkcs_padding, &new_fecf);
  if (status != CRYPTO_LIB_SUCCESS) {
    return status;
  }
...
/** Preserving for now
    ...
    if (badIV == 1)
    {
       * (sa_ptr->iv + sa_ptr->shivf_len - 1) =* (sa_ptr->iv + sa_ptr->shivf_len
- 1) + 1;
    }
    if (badMAC == 1)
    {
    ...
    // Header
    tempTM[count++] = (uint8_t)((tm_frame.tm_header.tfvn << 6) |
((tm_frame.tm_header.scid & 0x3F0) >> 4)); printf("LINE: %d\n",__LINE__);
    tempTM[count++] = (uint8_t)(((tm_frame.tm_header.scid & 0x00F) << 4) |
(tm_frame.tm_header.vcid << 1) | (tm_frame.tm_header.ocff)); tempTM[count++] =
(uint8_t)(tm_frame.tm_header.mcfc); tempTM[count++] =
(uint8_t)(tm_frame.tm_header.vcfc); tempTM[count++] =
        (uint8_t)((tm_frame.tm_header.tfsh << 7) | (tm_frame.tm_header.sf << 6)
| (tm_frame.tm_header.pof << 5) | (tm_frame.tm_header.slid << 3) |
((tm_frame.tm_header.fhp & 0x700) >> 8)); tempTM[count++] =
(uint8_t)(tm_frame.tm_header.fhp & 0x0FF);
    //	tempTM[count++] = (uint8_t) ((tm_frame.tm_header.tfshvn << 6) |
tm_frame.tm_header.tfshlen);
    ...
    // TODO: Troubleshoot
    // Padding Length
    // pad_len = Crypto_Get_tmLength(*len_ingest) - TM_MIN_SIZE + IV_SIZE +
TM_PAD_SIZE -*len_ingest; printf("LINE: %d\n",__LINE__);
...
ESA testing printf("LINE: %d\n",__LINE__); tempTM[count++] = 0x00; // pad_len >>
8; tempTM[count++] = 0x1A; // pad_len pdu_loc = count; pdu_len =*len_ingest +
pad_len;
    }
    printf("LINE: %d\n",__LINE__);
    // Payload Data Unit
    ...
    else if ((sa_ptr->est == 1) && (sa_ptr->ast == 1))
    {
        ...
        status = cryptography_if->cryptography_aead_encrypt(&(ingest[pdu_loc]),
// ciphertext output (size_t)pdu_len,            // length of data
                                                           &(tempTM[pdu_loc]),
// plaintext input (size_t)pdu_len,             // in data length
                                                           &(ekp->value[0]), //
Key KEY_SIZE, sa_ptr, sa_ptr->iv, sa_ptr->shivf_len,
                                                           &(ingest[mac_loc]),
                                                           MAC_SIZE,
                                                           &(aad[0]), // AAD
Input location sa_ptr->abm_len, // AAD is size of ABM in this case CRYPTO_TRUE,
// Encrypt CRYPTO_FALSE, // Authenticate // TODO -- Set to SA value, manually
setting to false here so existing tests pass. Existing data was generated with
authenticate then encrypt, when it should have been encrypt then authenticate.
                                                           CRYPTO_TRUE, // Use
AAD sa_ptr->ecs, // encryption cipher sa_ptr->acs,  // authentication cipher
                                                           NULL // cam_cookies
(not supported in TM functions yet)
                                                           );
        ...
        // Update FECF
        tm_frame.tm_sec_trailer.fecf = Crypto_Calc_FECF((uint8_t*)ingest,
fecf_loc - 1); ingest[fecf_loc] = (uint8_t)((tm_frame.tm_sec_trailer.fecf &
0xFF00) >> 8); ingest[fecf_loc + 1] = (uint8_t)(tm_frame.tm_sec_trailer.fecf &
0x00FF);
    }
    // Authentication
    else if ((sa_ptr->est == 0) && (sa_ptr->ast == 1))
 ...
 * Sets up TM Process Security.  Verifies ingest length, verifies pointers are
 *not null, Retreives managed parameters,  validates GVCID, and verifies the
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Process_Setup(uint16_t len_ingest, uint16_t *byte_idx,
                                uint8_t *p_ingest, uint8_t *secondary_hdr_len) {
  int32_t status = CRYPTO_LIB_SUCCESS;
#ifdef DEBUG
  ...
  if (len_ingest <
      6) // Frame length doesn't even have enough bytes for header -- error out.
  {
  ...
  if ((sa_if == NULL) &&
      (status == CRYPTO_LIB_SUCCESS)) // This should not happen, but tested here
                                      // for safety
  {
    printf(KRED "ERROR: SA DB Not initalized! -- CRYPTO_LIB_ERR_NO_INIT, Will "
                "Exit\n" RESET);
  ...
  printf(KGRN "tvfn: %d\t scid: %d\t vcid: %d\n" RESET, tm_frame_pri_hdr.tfvn,
         tm_frame_pri_hdr.scid, tm_frame_pri_hdr.vcid);
#endif
  ...
  } // Unable to get necessary Managed Parameters for TM TF -- return with
    // error.
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Determine_Cipher_Mode(uint8_t sa_service_type,
                                        SecurityAssociation_t *sa_ptr,
...
int32_t Crypto_TM_FECF_Setup(uint8_t *p_ingest, uint16_t len_ingest) {
  ...
  if (current_managed_parameters_struct.has_fecf == TM_HAS_FECF) {
    uint16_t received_fecf =
        (((p_ingest[current_managed_parameters_struct.max_frame_size - 2]
           << 8) &
          0xFF00) |
         (p_ingest[current_managed_parameters_struct.max_frame_size - 1] &
          0x00FF));
...
#ifdef TM_DEBUG
    printf(KRED "TM_Process Error...tfvn: %d scid: 0x%04X vcid: 0x%02X "
                "fecf_enum: %d\n" RESET,
           current_managed_parameters_struct.tfvn,
           current_managed_parameters_struct.scid,
           current_managed_parameters_struct.vcid,
 ...
 * @return int32_t: Success/Failure
 **/
int32_t Crypto_TM_Parse_Mac_Prep_AAD(uint8_t sa_service_type, uint8_t *p_ingest,
                                     int mac_loc, SecurityAssociation_t *sa_ptr,
                                     uint16_t *aad_len, uint16_t byte_idx,
                                     uint8_t *aad) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if ((sa_service_type == SA_AUTHENTICATION) ||
      (sa_service_type == SA_AUTHENTICATED_ENCRYPTION)) {
 ...
 * Performs decryption on AEAD Authentication, Encryption, and Authenticated
 * Encryption
 * @param sa_service_type: uint8_t
 * @param p_ingest: uint8_t*
 ...
 * @return int32_t: Success/Failure
 */
int32_t Crypto_TM_Do_Decrypt_AEAD(uint8_t sa_service_type, uint8_t *p_ingest,
                                  uint8_t *p_new_dec_frame, uint16_t byte_idx,
                                  uint16_t pdu_len, crypto_key_t *ekp,
                                  SecurityAssociation_t *sa_ptr, uint8_t iv_loc,
                                  int mac_loc, uint16_t aad_len, uint8_t *aad) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_service_type == SA_ENCRYPTION) {
    status = cryptography_if->cryptography_decrypt(
        p_new_dec_frame + byte_idx, // plaintext output
 ...
 * @return int32_t: Success/Failure
 */
int32_t Crypto_TM_Do_Decrypt_NONAEAD(uint8_t sa_service_type, uint16_t pdu_len,
                                     uint8_t *p_new_dec_frame,
                                     uint16_t byte_idx, uint8_t *p_ingest,
                                     crypto_key_t *akp, crypto_key_t *ekp,
                                     SecurityAssociation_t *sa_ptr,
                                     uint8_t iv_loc, int mac_loc,
                                     uint16_t aad_len, uint8_t *aad) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_service_type == SA_AUTHENTICATION ||
      sa_service_type == SA_AUTHENTICATED_ENCRYPTION) {
 ...
 * @return int32_t: Success/Failure
 */
void Crypto_TM_Calc_PDU_MAC(uint16_t *pdu_len, uint16_t byte_idx,
                            SecurityAssociation_t *sa_ptr, int *mac_loc) {
  *pdu_len = current_managed_parameters_struct.max_frame_size -
 ...
 * @return int32_t: Success/Failure
 */
int32_t Crypto_TM_Do_Decrypt(
    uint8_t sa_service_type, SecurityAssociation_t *sa_ptr,
    uint8_t ecs_is_aead_algorithm, uint16_t byte_idx, uint8_t *p_new_dec_frame,
    uint16_t pdu_len, uint8_t *p_ingest, crypto_key_t *ekp, crypto_key_t *akp,
    uint8_t iv_loc, int mac_loc, uint16_t aad_len, uint8_t *aad,
    uint8_t **pp_processed_frame, uint16_t *p_decrypted_length) {
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_service_type != SA_PLAINTEXT && ecs_is_aead_algorithm == CRYPTO_TRUE) {
    status = Crypto_TM_Do_Decrypt_AEAD(sa_service_type, p_ingest,
                                       p_new_dec_frame, byte_idx, pdu_len, ekp,
                                       sa_ptr, iv_loc, mac_loc, aad_len, aad);
  }

  else if (sa_service_type != SA_PLAINTEXT &&
           ecs_is_aead_algorithm == CRYPTO_FALSE) {
    status = Crypto_TM_Do_Decrypt_NONAEAD(
        sa_service_type, pdu_len, p_new_dec_frame, byte_idx, p_ingest, akp, ekp,
        sa_ptr, iv_loc, mac_loc, aad_len, aad);
    // TODO - implement non-AEAD algorithm logic
  }
 ...
 * @param sa_ptr: SecurityAssociation_t*
 */
void Crypto_TM_Process_Debug_Print(uint16_t byte_idx, uint16_t pdu_len,
                                   SecurityAssociation_t *sa_ptr) {
 ...
 * @return int32: Success/Failure
 **/
int32_t Crypto_TM_ProcessSecurity(uint8_t *p_ingest, uint16_t len_ingest,
                                  uint8_t **pp_processed_frame,
  ...
  tm_frame_pri_hdr.scid = (((uint16_t)p_ingest[0] & 0x3F) << 4) |
                          (((uint16_t)p_ingest[1] & 0xF0) >> 4);
  tm_frame_pri_hdr.vcid = ((uint8_t)p_ingest[1] & 0x0E) >> 1;

  status = Crypto_TM_Process_Setup(len_ingest, &byte_idx, p_ingest,
                                   &secondary_hdr_len);
  if (status == CRYPTO_LIB_SUCCESS) {
    /**
     * Begin Security Header Fields
  ...
  if (status == CRYPTO_LIB_SUCCESS) {
    // Determine Algorithm cipher & mode. // TODO - Parse authentication_cipher,
    // and handle AEAD cases properly
    status = Crypto_TM_Determine_Cipher_Mode(
        sa_service_type, sa_ptr, &encryption_cipher, &ecs_is_aead_algorithm);
  }
  if (status == CRYPTO_LIB_SUCCESS) {
#ifdef TM_DEBUG
    switch (sa_service_type) {
  ...
  if (status == CRYPTO_LIB_SUCCESS) {
    ...
    Crypto_TM_Parse_Mac_Prep_AAD(sa_service_type, p_ingest, mac_loc, sa_ptr,
                                 &aad_len, byte_idx, aad);

    status = Crypto_TM_Do_Decrypt(
        sa_service_type, sa_ptr, ecs_is_aead_algorithm, byte_idx,
        p_new_dec_frame, pdu_len, p_ingest, ekp, akp, iv_loc, mac_loc, aad_len,
        aad, pp_processed_frame, p_decrypted_length);
  }

  return status;
}

void Crypto_TM_Print_FSR(uint8_t *p_ingest, uint16_t byte_idx, uint16_t pdu_len,
                         SecurityAssociation_t *sa_ptr) {
  if (current_managed_parameters_struct.has_ocf == TM_HAS_OCF) {
    byte_idx += (pdu_len + sa_ptr->stmacf_len);
...
int32_t Crypto_Get_tmLength(int len) {
  ...
  len = TM_FILL_SIZE;
#else
  len = TM_FRAME_PRIMARYHEADER_SIZE + TM_FRAME_SECHEADER_SIZE + len +
        TM_FRAME_SECTRAILER_SIZE + TM_FRAME_CLCW_SIZE;
#endif
...
{ // Copy ingest to PDU
    ...
    // Consider a helper function here, or elsewhere, to do all the 'math' in
one spot as a global accessible list of variables if
(sa_if->sa_get_from_spi(tm_frame[0], &sa_ptr) != CRYPTO_LIB_SUCCESS) // modify
    {
        // TODO - Error handling
...
tm_offset]); printf("%02X", (uint8_t)ingest[x - tm_offset]);
...
void Crypto_TM_updateOCF(Telemetry_Frame_Ocf_Fsr_t *report, TM_t *tm_frame) {
  // TODO
  tm_frame->tm_sec_trailer.ocf[0] = (report->cwt << 7) | (report->fvn << 4) |
                                    (report->af << 3) | (report->bsnf << 2) |
                                    (report->bmacf << 1) | (report->bsaf);
 ...
 **/
uint32_t Crypto_Prepare_TM_AAD(const uint8_t *buffer, uint16_t len_aad,
                               const uint8_t *abm_buffer, uint8_t *aad) {
