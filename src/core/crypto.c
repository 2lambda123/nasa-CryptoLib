...
AOS_FramePrimaryHeader_t
    aos_frame_pri_hdr; // Used to reduce bit math duplication
 ...
 * Looks up cipher suite ID and determines if it's an AEAD algorithm. Returns 1
 *if true, 0 if false;
 * @param cipher_suite_id: uint32
 **/
uint8_t Crypto_Is_AEAD_Algorithm(uint32_t cipher_suite_id) {
  // CryptoLib only supports AES-GCM, which is an AEAD (Authenticated Encryption
  // with Associated Data) algorithm, so return true/1.
  // TODO - Add cipher suite mapping to which algorithms are AEAD and which are
  // not.
  if ((cipher_suite_id == CRYPTO_CIPHER_AES256_GCM) ||
      (cipher_suite_id == CRYPTO_CIPHER_AES256_CBC_MAC) ||
...
int32_t Crypto_window(uint8_t *actual, uint8_t *expected, int length,
  ...
  // Check for special case where received value is all 0's and expected is all
  // 0's (won't have -1 in sa!) Received ARSN is: 00000000, SA ARSN is: 00000000
  ...
  for (i = 0; i < window; i++) {
    // Recall - the stored IV or ARSN is the last valid one received, check
    // against next expected
...
uint8_t Crypto_Prep_Reply(uint8_t *ingest, uint8_t appID) {
  ...
  ingest[count++] = (sdls_frame.hdr.pvn << 5) | (sdls_frame.hdr.type << 4) |
                    (sdls_frame.hdr.shdr << 3) |
                    ((sdls_frame.hdr.appID & 0x700 >> 8));
  ingest[count++] = (sdls_frame.hdr.appID & 0x00FF);
  ingest[count++] =
      (sdls_frame.hdr.seq << 6) | ((sdls_frame.hdr.pktid & 0x3F00) >> 8);
  ...
  // ingest[count++] = (sdls_frame.pus.shf << 7) | (sdls_frame.pus.pusv << 4) |
  // (sdls_frame.pus.ack); ingest[count++] = (sdls_frame.pus.st);
  // ingest[count++] = (sdls_frame.pus.sst);
  // ingest[count++] = (sdls_frame.pus.sid << 4) | (sdls_frame.pus.spare);

  // Fill ingest with Tag and Length
  ingest[count++] = (sdls_frame.pdu.type << 7) | (sdls_frame.pdu.uf << 6) |
                    (sdls_frame.pdu.sg << 4) | (sdls_frame.pdu.pid);
 ...
 * Calculate the Frame Error Control Field (FECF), also known as a cyclic
...
int32_t Crypto_FECF(int fecf, uint8_t* ingest, int len_ingest,TC_t* tc_frame)
{
    ...
    if ( (fecf & 0xFFFF) != calc_fecf )
        {
            if (((uint8_t)ingest[18] == 0x0B) && ((uint8_t)ingest[19] == 0x00)
&& (((uint8_t)ingest[20] & 0xF0) == 0x40))
            {
...
calc_fecf, tc_frame->tc_sec_trailer.fecf); #endif result = CRYPTO_LIB_ERROR;
            }
        }
...
uint16_t Crypto_Calc_FECF(const uint8_t *ingest, int len_ingest) {
  ...
  uint16_t poly = 0x1021; // TODO: This polynomial is (CRC-CCITT) for ESA
                          // testing, may not match standard protocol
  uint8_t bit;
  uint8_t c15;
  int i;
  int j;

  for (i = 0; i < len_ingest; i++) { // Byte Logic
    for (j = 0; j < 8; j++) {        // Bit Logic
  ...
  int x;
  printf(KCYN
         "Crypto_Calc_FECF: 0x%02x%02x%02x%02x%02x, len_ingest = %d\n" RESET,
         ingest[0], ingest[1], ingest[2], ingest[3], ingest[4], len_ingest);
 ...
 * @return uint16: CRC
 **/
uint16_t Crypto_Calc_CRC16(uint8_t *data, int size) { // Code provided by ESA
  uint16_t crc = 0xFFFF;
...
int32_t Crypto_PDU(uint8_t *ingest, TC_t *tc_frame) {
  ...
  case 0: // Command
    ...
    case 0: // CCSDS Defined Command
      ...
      case SG_KEY_MGMT: // Key Management Procedure
        ...
        case PID_KEY_INVENTORY:
          ...
          break;
        default:
          printf(KRED "Error: Crypto_PDU failed interpreting Key Management "
                      "Procedure Identification Field! "
                      "\n" RESET);
          break;
        }
        break;
      case SG_SA_MGMT: // Security Association Management Procedure
        ...
        case PID_SA_STATUS:
          ...
          break;
        default:
          printf(KRED "Error: Crypto_PDU failed interpreting SA Procedure "
      ...
      case SG_SEC_MON_CTRL: // Security Monitoring & Control Procedure
        ...
        case PID_ALARM_FLAG:
          ...
          break;
        default:
          printf(KRED "Error: Crypto_PDU failed interpreting MC Procedure "
                      "Identification Field! \n" RESET);
          break;
        }
        break;
      default: // ERROR
        printf(KRED
               "Error: Crypto_PDU failed interpreting Service Group! \n" RESET);
        break;
      }
      break;
    ...
    case 1: // User Defined Command
      ...
      default:
        ...
        default:
          printf(KRED
                 "Error: Crypto_PDU received user defined command! \n" RESET);
          break;
        }
      }
      break;
  ...
  case 1: // Reply
    printf(KRED "Error: Crypto_PDU failed interpreting PDU Type!  Received a "
                "Reply!?! \n" RESET);
    break;
  }
...
int32_t Crypto_Get_Managed_Parameters_For_Gvcid(
  ...
  for (int i = 0; i < gvcid_counter; i++) {
    if (managed_parameters_in[i].tfvn == tfvn &&
        managed_parameters_in[i].scid == scid &&
  ...
  if (status != CRYPTO_LIB_SUCCESS) {
    printf(KRED "Error: Managed Parameters for GVCID(TFVN: %d, SCID: %d, VCID: "
                "%d) not found. \n" RESET,
           tfvn, scid, vcid);
  }
 ...
 * @return int32: Success/Failure
 **/
// int32_t Crypto_Get_Managed_Parameters_For_Gvcid(uint8_t tfvn, uint16_t scid,
// uint8_t vcid,
//                                                 GvcidManagedParameters_t*
//                                                 managed_parameters_in,
//                                                 GvcidManagedParameters_t**
...
//     {
//         if (managed_parameters_in->tfvn == tfvn &&
//         managed_parameters_in->scid == scid &&
...
//         else
//         {
//             return Crypto_Get_Managed_Parameters_For_Gvcid(tfvn, scid, vcid,
//             managed_parameters_in->next,
//                                                            managed_parameters_out);
//         }
//     }
//     else
//     {
//         printf(KRED "Error: Managed Parameters for GVCID(TFVN: %d, SCID: %d,
//         VCID: %d) not found. \n" RESET, tfvn, scid,
//                vcid);
//         return status;
//     }
 ...
 * Managed parameters are expected to live the duration of the program, this may
 *not be necessary.
...
int32_t Crypto_Process_Extended_Procedure_Pdu(TC_t *tc_sdls_processed_frame,
  ...
  if (crypto_config.has_pus_hdr == TC_HAS_PUS_HDR) {
    if ((tc_sdls_processed_frame->tc_pdu[0] == 0x18) &&
        (tc_sdls_processed_frame->tc_pdu[1] == 0x80))
    // Crypto Lib Application ID
    {
      ...
      sdls_frame.hdr.appID =
          ((tc_sdls_processed_frame->tc_pdu[0] & 0x07) << 8) |
          tc_sdls_processed_frame->tc_pdu[1];
      sdls_frame.hdr.seq = (tc_sdls_processed_frame->tc_pdu[2] & 0xC0) >> 6;
      sdls_frame.hdr.pktid =
          ((tc_sdls_processed_frame->tc_pdu[2] & 0x3F) << 8) |
          tc_sdls_processed_frame->tc_pdu[3];
      sdls_frame.hdr.pkt_length = (tc_sdls_processed_frame->tc_pdu[4] << 8) |
                                  tc_sdls_processed_frame->tc_pdu[5];
      ...
      sdls_frame.pdu.pdu_len = (tc_sdls_processed_frame->tc_pdu[11] << 8) |
                               tc_sdls_processed_frame->tc_pdu[12];
  ...
  } else if (tc_sdls_processed_frame->tc_header.vcid ==
             TC_SDLS_EP_VCID) // TC SDLS PDU with no packet layer
  {
#ifdef DEBUG
    printf(KGRN "Received SDLS command: " RESET);
#endif
    ...
    sdls_frame.pdu.pdu_len = (tc_sdls_processed_frame->tc_pdu[1] << 8) |
                             tc_sdls_processed_frame->tc_pdu[2];
    for (x = 3; x < (3 + tc_sdls_processed_frame->tc_header.fl); x++) {
      // Todo - Consider how this behaves with large OTAR PDUs that are larger
      // than 1 TC in size. Most likely fails. Must consider Uplink Sessions
      // (sequence numbers).
...
int32_t Crypto_Check_Anti_Replay_Verify_Pointers(SecurityAssociation_t *sa_ptr,
  ...
  int32_t status = CRYPTO_LIB_SUCCESS;
  if (sa_ptr == NULL) // #177 - Modification made per suggestion of 'Spicydll' -
...
int32_t Crypto_Check_Anti_Replay_GCM(SecurityAssociation_t *sa_ptr, uint8_t *iv,
  ...
  if ((sa_ptr->iv_len > 0) && (sa_ptr->ecs == CRYPTO_CIPHER_AES256_GCM)) {
    ...
    {
      // Whole IV gets checked in MAC validation previously, this only verifies
      // transmitted portion is what we expect.
      status =
          Crypto_window(iv, sa_ptr->iv + (sa_ptr->iv_len - sa_ptr->shivf_len),
...
int32_t Crypto_Check_Anti_Replay(SecurityAssociation_t *sa_ptr, uint8_t *arsn,
  ...
  if ((sa_ptr->ecs == CRYPTO_CIPHER_AES256_GCM ||
       sa_ptr->ecs == CRYPTO_CIPHER_AES256_GCM_SIV) &&
      (iv_valid == CRYPTO_TRUE)) {
...
int32_t Crypto_Get_Security_Header_Length(SecurityAssociation_t *sa_ptr) {
  ...
  ** eventually we need a way to reconcile cryptolib managed parameters with TO
  managed parameters GvcidManagedParameters_t* temp_current_managed_parameters =
  NULL; Crypto_Get_Managed_Parameters_For_Gvcid(tfvn, scid, vcid,
                                          gvcid_managed_parameters,
  temp_current_managed_parameters);
  */
